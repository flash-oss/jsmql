...
- `bson.ts` is the one module that names `bson` — a PEER dependency, so the value jsmql emits is the caller's own class. It builds with the real classes and recognises with `instanceof` OR the `_bsontype` tag, so a value from a second copy still counts. It also re-exports the registry's realm-independent recognisers (`isDate`, `isRegExp`, `isBytes`, `isPlainObject`): no module tests a value with `instanceof Date` or against `Object.prototype`, because a value from another realm fails both — `test/cross-realm.test.ts` holds the line. See docs/specs/bson-types.md § Recognition across realms.
