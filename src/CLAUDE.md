...
- **`$`-string literals: pipeline pass-through vs `jsmql.expr` wrap.** A user-written string literal that starts with `$` (e.g. `"$items"`) is wrapped in `{ $literal: … }` by `literalSafeString` so MongoDB doesn't read it as a field ref — but ONLY in the `jsmql.expr` expression surface. In pipeline/stage context the wrap is suppressed by `GenerateCtx.pipelineContext` (seeded once in `generatePipeline` / `generateImplicitPipeline` / `generatePipelineWithCtx`, propagated through `extendCtx` / `freshSubPipelineCtx` / `freshFacetCtx`), so `$`-strings pass through verbatim everywhere in a pipeline — including inside nested operator args. This makes pasted raw MQL round-trip. The flag is set by *surface* and never flipped at operator boundaries (the deliberate "Model B" choice; "Model A" rejected in DEFERRED §B). `$literal(...)` (which sets `insideLiteral`) is the escape hatch to force a literal inside a pipeline. See `docs/specs/aggregation-stages.md`.
