...
Two reviewers' tools for the suites, run by hand and never on a hook, over the half they share in `expectations.mjs`. `regen-expectations.mjs test/<suite>.test.ts` rewrites every `expect(<call>).toEqual(<literal>)` (and `toStrictEqual` / `toBe` / `toThrow(<matcher>)`) with the answer the working-tree compiler gives for the same call, through the TypeScript AST — for a deliberate change of emitted shape that would otherwise be a hundred identical hand edits. A call whose polarity changed (a throw where a value was asserted, or a value where a throw was) is listed and left alone. `convert-expectations.mjs test/<suite>.test.ts ['<keep regex>' …]` is the mechanical half of that judgement: it flips the polarity of every listed case except those whose source matches a KEEP pattern — the refusals the suite must keep asserting. Both outputs are reviewed as a diff before they are committed: a wrong answer regenerates as well as a right one, so the script proves consistency with the compiler, never correctness (a running `mongod` proves that — see [test/CLAUDE.md](../test/CLAUDE.md)).

`expectations.mjs` holds what both need: the suite file read as a TypeScript AST, an expression evaluated in the scope the suite itself builds (its own top-level constants, plus the ones declared above the call in every enclosing block), `expectCall` / `literalSubject` to recognise an assertion, and `spell` — which is `jsmql.stringify`, so a regenerated expectation reads exactly as the CLI prints. `undefined` is the one value the printer refuses that a suite may still assert (`expect(doc.let).toEqual(undefined)` states a key is absent), and `spell` writes it. After the edits land the file goes through `oxfmt`, so the formatter owns the layout and the diff holds the answers that changed and no re-wrapped line that did not.

### `check-doc-claims.mjs`

`node scripts/check-doc-claims.mjs [file …]` (default: `README.md`, `docs/LANGUAGE.md`, `docs/LANG_RULES.md`, and every file in `docs/specs/`) re-derives every `<jsmql source>  // → <MQL>` pair in the prose from the compiler and prints the pairs that disagree. A doc example is a promise about what jsmql emits, and prose has no test to keep it honest — this is what catches the promise the compiler stopped keeping. An AUDIT tool, not a gate: it parses markdown, so it reports false positives — a template tag that interpolates a value, a claim showing one stage of a longer pipeline, host code around a `jsmql(…)` call — and a human classifies each. A claim may share its source's line (`$.a + $.b   // → { $add: … }`) or follow it; a trailing `// note` on a claim line, a trailing comma before a closing bracket, and prose after the shape are ignored. The source may be a quoted string, a template tag without `${…}`, or a `jsmql.stringify(<call>)` around either; a `jsmql.validate(…)` claim is held to what `validate` returns. It skips a claim that elides anything (`…`, `/* … */`, `<…>`) or opens on a key rather than a bracket (`let: { … }`), which is illustrative by construction. The compiler's answer is printed with `jsmql.stringify`, the same printer the docs' examples are written from, so a claim that spells a Date or an ObjectId compares as written; quoting and spacing are normalised away on both sides.
