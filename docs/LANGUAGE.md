...
| `.map(d => <expr>)` | Single-param expression-body arrow; the param is the current document (write `d.x`, not `$.x`). Embedded `$$$.<coll>.find/filter(...)` lookups are allowed in the top-level `$$` chain | `$replaceWith: <expr>` — the chain-form of `$ = <expr>`. Embedded lookups materialise into prologue `$lookup` stages ahead of the `$replaceWith`. In the `$$$.<coll>.<chain>` lookup-body context, embedded lookups are rejected (would be nested inside `$unionWith.pipeline` — hoist instead) |
