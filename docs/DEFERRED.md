...
- **What's blocked (narrowed).** `$$.length` (= the ROOT stream count) is now supported **inside a top-level `$lookup`** — its predicate (`$$$.coll.filter(o => o.n === $$.length)`), block body, and `.map` chain — by capturing the top-materialised `$__jsmql.length` into the `$lookup.let` as `v0_length` and reading it back as `$$v0_length`. Inner *sub-stream* counts ship too, via the named 3rd-arg handle (`.map((o, _i, coll) => coll.length)`). **Still blocked:** `$$.length` inside a `$facet` / `$unionWith` sub-pipeline; `$$.length` *deeper* than one `$lookup` level (capture is gated to `depth === 0`); and `$$.length` inside a **reusable function body** (`const f = () => $$.length`).
