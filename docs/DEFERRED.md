...
- **Partial progress (2026-08-01).** Chained stage calls (`$$$.<coll>.$out(…)`) DO get the forbidden-in check: at each of the three stage-link sites the container is unambiguous, so `checkStageLinkPlacement` in `src/stage-link.ts` applies `forbiddenIn` / `position` directly without solving the general `lowerBlock` threading problem. The remaining gap is the `.aggregate((o) => { … })` / predicate block-body path described above.
