...
- **What's blocked.** A literal *diagnostic* stage — one that a *single* container forbids (`forbiddenIn: ["facet"]`) — written inside a *sugar predicate block-body* lambda: `$ = { k: $$.filter(d => { { $collStats: {} }; return d.a; }) }`. It gets must-first / must-last validation but NOT the forbidden-in-`$facet` ban. The all-container write stages (`$out` / `$merge`) ARE covered in every block body; so are literal sub-pipeline arrays (`{ $lookup: { pipeline: [...] } }`), via `generatePipelineWithCtx(container)`.
...
- **Success criteria.** A single-container stage in a block body throws its container-named error with a meaningful `.pos` — `$ = { k: $$.filter(d => { { $collStats: {} }; return d.a; }) }` names `$facet`. Tests in `test/stage-validation.test.ts` / `test/pipeline.test.ts`.
