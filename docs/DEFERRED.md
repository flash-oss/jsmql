...
- **Why blocked.** Contorted and rarely needed on a stream — the running-flag `$setWindowFields` + boundary `$match` is heavy, and the far more common intent (take while sorted-key < X) is already expressible as `.sort(<key>)` + `.filter(o => o.<key> < X)`. Deferred by developer decision (2026-07-19) after implementing the rest of the lodash stream Tier 3 (`.takeRight`/`.dropRight`/`.initial`/`.shuffle`) — three of which were later **removed** from the stream surface (2026-08-01, see DEVLOG): a stream has no order to count back from. `.takeWhile`/`.dropWhile` are unaffected by that reasoning (they run from the FRONT of an order a preceding `.sort(...)` establishes), but the "or default to `_id`" fallback in the success criteria below is **no longer acceptable** — that silent substitution is exactly what got the from-the-end family removed. If these ever ship, they must require an explicit preceding sort and reject otherwise.
