...
- Every predicate position on an array-value method routes through `jsBool`, whichever spelling the predicate takes: the JavaScript predicate methods and the bare `Boolean` callback (`jsBoolIfNeeded` at the lambda body), the lodash predicate-run family (`resolvePredicate` in `src/codegen.ts` — see `method-dispatch.md`), and `.compact()`, whose `$filter` `cond` **is** a `jsBool` of the element. One rule everywhere is what makes `.compact()` identical to `.filter(Boolean)` and `.reject(p)` the exact complement of `.filter(p)`.
