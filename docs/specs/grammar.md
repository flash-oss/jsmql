...
- **Expression block** (`expr_block` above) — the default everywhere a lambda is a value (array methods, `$let`, IIFE, `Array.from`). It is `(const|let <name> = <expr>;)* return <expr>;` and lowers to a right-folded nest of `$let` (see [emit-pass.md → Block-body arrows](emit-pass.md#block-body-arrows--nested-let)). A bare `=> { k: v }` is rejected (no `return`), pointing at `=> ({ k: v })`; re-declaring a name, or omitting `return`, are likewise actionable errors.
