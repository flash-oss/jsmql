...
The methods derive from the registries, so adding a new diagnostic stage (a `diagnostic` field on a `STAGES` entry), a new stage, or a new stream method surfaces it on the right ref automatically. A new diagnostic stage needs a `DIAGNOSTIC_OPTION_SHAPES` entry only if it takes options; a new stream method needs a `STREAM_METHOD_SIGNATURES` entry. `streamMethodMembers()` guards that table in **both** directions — every registry name must have a signature, and every signature must be a live registry name, so a method jsmql later drops from streams cannot linger as a phantom completion.
