...
A **value** method — one the `METHODS` registry in [`src/compiler/emit/lower.ts`](../../src/compiler/emit/lower.ts) recognises on an array / string / number / date *value* (`$.items.uniq()`, `$.name.capitalize()`, `$.placedAt.startOf("month")`) rather than on a `$`-prefixed global. Completion therefore needs the *receiver* to have a real type, so `valueMethodAugmentationBlock()` augments the matching built-in interface with each one. The receiver interfaces it emits are the keys of `VALUE_METHOD_INTERFACES`, and that map is also the emit order.
