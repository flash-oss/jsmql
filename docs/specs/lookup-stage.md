...
- **Terminal** (the *last* method): peeled off (`peelableTerminalMap`) and returned as `<slot>.map(iteratee)` in `rewritten`, so codegen lowers it to `$map` over the result array.
...
| `$$$.<coll>.<other-method>(...)` (e.g. `.fnid(...)`)    | `'$$$.<coll>' supports .find(pred), .filter(pred), and .aggregate(pipeline), not .<m>(). Did you mean '.find'?` | `validateLookupShape`              |
| `$$$$.<db>.<coll>.<other-method>(...)`                  | `'$$$$.<db>.<coll>' supports .find(pred), .filter(pred), and .aggregate(pipeline), not .<m>(). Did you mean '.find'?` | `validateLookupShape`        |
