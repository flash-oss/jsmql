...
**Two head shapes in `tryExtractChainedLookup`.** A `.filter` head seeds the sub-pipeline from its predicate and peels from `methods[1]` (preserving the `let: {}` output). A stream-method / `.reject` head resolves the target via `extractLookupTarget(cur)`, seeds EMPTY, and peels from `methods[0]`; with no correlation it emits the lean `{ from, pipeline, as }` (no empty `let`, matching the `.aggregate` shape). `.find`/`.aggregate` heads keep their existing dedicated handling.
...
**Lowering.** Always pipeline-form (never basic-form): `lowerLookup` emits `{ $lookup: { from, [let,] pipeline, as } }` and **omits an empty `let`** for the uncorrelated case (`{ from, pipeline, as }` — an empty `let` is noise). Result is an **array** (like `.filter`), so no `$set { $first }`.
