...
2. **Predicate translation.** `translatePredicate(call, ctx, lowerBlock)` picks between the basic form (`{ from, localField, foreignField, as }`) and the correlated-pipeline form (`{ from, let, pipeline, as }`). The basic-form fast path is taken when the lambda body is exactly one `===` between a foreign-rooted path (`o.x.y`) and a `$.` local-rooted path — and because the shorthands are normalised to arrows at detection (§1), `.filter({ userId: $._id })` and `["userId", $._id]` reach it as the same `===` body and earn the same indexed basic form. Predicate **spelling never changes the emitted MQL**; only predicate *shape* does. Anything richer — including `==` (which jsmql restricts to `null` comparisons project-wide; see LANGUAGE.md's `===` vs `==` table) — falls through to the pipeline form, where the sub-pipeline codegen then surfaces the standard "use `===`" rejection if the user wrote `==` between two field paths. A block-body lambda that keeps statements of its own uses the pipeline form, with the rewritten block as the `pipeline:` body; one that is only `return <expr>` is canonicalised to its expression form first (see § Canonical predicate form) and so is eligible for the basic form exactly as the arrow spelling is. The **expression-body** pipeline form routes the let-extracted predicate through `translateMatchBody` + `matchStagesFromTranslation` — the same index-friendly emitter the sibling translators use (see below) — so a constant comparison (`o.status === "x"`) becomes an indexable `{ status: "x" }` query field and only the parts with no query form (a correlated `$$letVar` comparison, a computed expression) fall back to `$expr`. (Previously this path wrapped the *whole* predicate in `$expr`; block-body lambdas already got the index-friendly form because each `$match(...)` statement runs through the same translator via `lowerBlock`.)
...
**Canonical predicate form (`canonicalPredicateLambda`).** A predicate's terminal
`return <expr>` **is** the predicate: in JavaScript `o => { return X; }` is the very same
function as `o => X`, so the two spellings must emit one shape. `canonicalPredicateLambda`
puts a predicate lambda in the form that guarantees the return is read:

- **`{ return X; }` alone** (either parse shape — a stage-less callback block, or the
  expression block `.reject` parses) → the expression lambda `o => X`. It then reaches the
  same basic-form detection (§2, the indexed `localField`/`foreignField` route), the same
  translation, and the same `.reject` negation the expression spelling gets.
- **statements + `return X`** → the block, with `X` appended as a synthetic `$match(X)`
  statement. The block is the meaning there, and as a statement the return rides the same
  parameter rewrite (`extractLetsFromPipeline`, so an outer `$.<field>` still captures into
  `$lookup.let`) and the same `$match` lowering — no second predicate path to drift from the
  first. A `const`/`let` in the block materialises as its binding stage, which the appended
  `$match` reads back through the namespace slot.
- The `.map` counterpart is `lowerCallbackBlock`'s `terminalRet`, appended as the root-replace
  `$ = <ret>`; `.aggregate` is exempt, its `return` reshaping the document rather than
  filtering it.

It is idempotent and cheap on the common path, so **every** entry point that receives a
predicate lambda calls it — the gates (`requireStreamPredicate`, `chainFilterLambda`,
`detectLookupCall`), both lowering sinks (`translatePredicate`,
`buildPipelineFormPredicate`), and the correlation probe (`predicateReferencesOuterDoc`).
Canonicalising at the sinks and not only at the gates is deliberate: a caller that
hand-rolls a `LookupCall` (the `$$ =` pivot builds one from the raw chain method) never
passes a gate, and a dropped return there is silent — the filter still emits, it just
matches everything.
...
Every pipeline-form `$lookup` body is assembled by the one exported `pipelineLookupBody(from, letVars, pipeline, as)` in `src/lookup-translation.ts`, which **omits `let` entirely when nothing was correlated** (`let` is optional to the server, and an empty `let: {}` is pure noise). Routing all five emission sites — the direct lookup, the chained assembler, and the three `pipeline.ts` pivots — through one helper is deliberate: re-deciding the rule per site would let `$$$.orders.take(2)` and `$$$.orders.filter(p).take(2)` disagree about `let: {}` for no semantic reason. The shape is a function of the *predicate*, never of which code path built it.
