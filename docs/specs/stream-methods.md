...
| `.toReversedBy("<field>")` | One plain field-name string literal (no leading `$`) | Descending key-sort — the key form of a descending `.toSorted` | One `{ $sort: { <field>: -1 } }` |
| `.sortBy(key \| [keys] \| { field: 1\|-1 })` | A field name, an array of field names (all ascending), or a raw `$sort` spec object (directions validated to 1/-1) | `buildSortBySpec` builds the spec; a computed arrow is rejected with a redirect to `.toSorted` | One `{ $sort: { … } }` |
| `.orderBy(keys, orders?)` | `keys`: a field name or array of them; `orders` (optional): `"asc"`/`"desc"` or an array of them, padded with `"asc"` to the key count | `buildOrderBySpec` zips keys with directions | One `{ $sort: { … } }` |
...
| `.uniqBy("<field>")` | One plain field-name string literal | `$group` keeping `$first` per key into the reserved `__jsmqlTmp` group slot, then `$replaceWith` to restore it. "First" follows the stream's current order — precede with `.sortBy`/`.toReversedBy` when which-duplicate-wins matters | `{ $group: { _id: "$<field>", __jsmqlTmp: { $first: "$$ROOT" } } }` + `{ $replaceWith: "$__jsmqlTmp" }`. Clears the let scope |
