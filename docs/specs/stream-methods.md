...
|---|---|---|
| **Field key** — `.sortBy` / `.orderBy` / `.groupBy` / `.countBy` / `.keyBy` / `.uniqBy` / `.flatMap` | the property string `"cat"`, or the equivalent bare-path arrow `d => d.cat` | `fieldKeyArg` (`src/stream-methods.ts`) |
...
**What a stream genuinely cannot take** is a *computed* key: the field key becomes a `$sort` key or a `$group._id`, which the server fixes at plan time. So a computed arrow (`d => d.cat.toLowerCase()`), and a matches-object wherever that spelling isn't already claimed by a richer surface, are rejected by the shared `computedKeyError` — which names the method it was called on and points at the way out (`$.key = <expr>;` then `.<method>("key")`, or a field-building `.map` inside a foreign chain). `.flatMap` rejects the same way, needing a path for `$unwind`. The object spelling **is** claimed, and so means something else, on three methods: `.orderBy({ field: dir })` and `.sort`/`.toSorted({ field: dir })` are direction specs, and `.groupBy({ _id, … })` is the `$group` body. `.sortBy({ … })` keeps a dedicated message pointing at those.
