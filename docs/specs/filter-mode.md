...
4. **A query document is read through an INDEX, so a dual-receiver method takes the indexable reading.** `.includes` means containment in an array and substring in a string, and the expression road emits both, gated on the value's own `$type`. A query slot takes MongoDB's own reading alone — `$.tags.includes("x")` → `{ tags: "x" }` — because a `$type`-gated `$or` cannot be planned. The substring test has its own query spelling, `.match(/x/)`, and `jsmql.expr` gives the two-reading form. `test/query-expr-agreement.test.ts` contracts this.
