...
5. **`||` lowers per branch.** Each branch is lowered on its own, so a native branch stays native beside an `$expr` branch: `$.tags === "red" || $.qty * $.price > 100` → `{ $or: [{ tags: "red" }, { $expr: { $gt: … } }] }`. A leaf's form never changes because of a sibling.
