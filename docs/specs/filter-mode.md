...
- **Auto-`$literal` wrap inside Filter values.** The wrap is applied in aggregation-expression codegen to prevent `"$x"`-looking strings from being parsed as field refs. The query language doesn't interpret values as field refs, so the wrap is **skipped** for values that land in the query-doc slot (e.g. `$.x === "$y"` → `{ x: "$y" }`). The wrap still applies inside the `$expr` residual where aggregation-expression codegen runs.
...
- **`this.<coll>` is Pipeline-only.** The `$lookup` shorthand (see [lookup-stage.md](lookup-stage.md)) can't appear in a Filter — `$lookup` is a pipeline stage. A no-`;` source containing `this.<coll>` reaches the codegen `ThisRef` case and throws an actionable error naming the `;`-terminated Pipeline shape it expected. `jsmql.filter()` produces the same error.
