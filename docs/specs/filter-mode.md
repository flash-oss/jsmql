...
3. **A JavaScript spelling becomes the plain query clause.** Every field-vs-constant pair the query language can express becomes the document a MongoDB developer reads and writes: `$.age > 18` → `{ age: { $gt: 18 } }`, `$.s === "a"` → `{ s: "a" }`, `$.x !== null` → `{ x: { $not: { $type: "null" } } }`. MongoDB's own rules then apply to it — a missing field counts as null, and an array matches element-wise — which is the boundary [LANG_RULES.md](../LANG_RULES.md) draws between JS syntax and MongoDB behaviour. The index on the field is used.
4. **A query document is read through an INDEX, so a dual-receiver method takes the indexable reading.** `.includes` means containment in an array and substring in a string, and the expression road emits both, gated on the value's own `$type`. A query slot takes MongoDB's own reading alone — `$.tags.includes("x")` → `{ tags: "x" }` — because a `$type`-gated `$or` cannot be planned. The substring test has its own query spelling, `.match(/x/)`, and `jsmql.expr` gives the two-reading form. Contracted in `test/query-expr-agreement.test.ts`.
5. **`&&` merges clauses on distinct fields into one document**; a repeated field, or a clause with no native form, rides in `$expr`.
6. **`||` lowers per branch.** Each branch is lowered on its own, so a native branch stays native beside an `$expr` branch: `$.tags === "red" || $.qty * $.price > 100` → `{ $or: [{ tags: "red" }, { $expr: { $gt: … } }] }`. A leaf's form never changes because of a sibling.
