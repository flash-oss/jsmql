...
- **Auto-`$literal` wrap inside Filter values.** The wrap is applied in aggregation-expression codegen to prevent `"$x"`-looking strings from being parsed as field refs. The query language doesn't interpret values as field refs, so the wrap is **skipped** for values that land in the query-doc slot (e.g. `$.x === "$y"` → `{ x: "$y" }`). The wrap still applies inside the **standalone** Filter `$expr` residual, which is generated without the `pipelineContext` flag (a standalone `db.coll.find(filter)` is neither a pipeline nor `jsmql.expr`; it keeps wrapping). Contrast a `$match` stage's `$expr` *inside a pipeline*, which is generated with `pipelineContext: true` and so passes `$`-strings through — see [aggregation-stages.md](aggregation-stages.md#-string-pass-through-in-pipeline-context-pipelinecontext). Unifying the standalone-Filter `$expr` with pipeline pass-through is deferred ([DEF-025](../DEFERRED.md)).
