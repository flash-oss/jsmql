...
- `certainReceiverType(object, ctx)` — the receiver's provable, invariant type, or `null` when uncertain. It reports every family the compiler can be sure of:
  - `bool` / `array` reuse the verified-sound `isProvablyBool` / `isArrayProducing`, which recurse through `.slice`/`MethodCall` and subsume literals as well as array-/bool-returning operator calls.
  - `string` / `number` / `object` come from the receiver **method's** invariant `returns`.
  - `date` from a date method that declares **no** `returns`, because that omission means "same-as-receiver" and the receiver is a date (`.plus`, `.minus`, `.startOf`, `.endOf`, `.set`); the date methods returning something else all declare it. A `NewDate` node is a date too.
  - Literal receivers: a string literal (unless it is an HR1 `$`-string, which IS a field reference and so stays uncertain), a number or bigint literal, a template literal, and `typeof` — all invariant.
  - `MemberAccess` on `length`, the one member whose type is invariant; every other member is a document field of unknown type.
  - `.pick` / `.omit` → `object` via `VALUE_MODE_OBJECT_METHODS`. They omit `returns` only because a `$$` stream reads them as a `$project` document stream, and reaching `generateMethodCall` *is* value position — a stream chain never gets here — so the value-mode answer is certain.
  - A **typed binding**, through `bindingTypeOf(o, ctx)` in the `default` branch: `ctx.bindingTypes` for a `const`/`let` whose value had a provable type, and `ctx.slotTypes` for a slot the compiler itself materialised (§ `bindingTypes`). This is what catches `const s = $.name.trim(); s.map(f)` — the binding carries the type its initialiser proved, so a chain on the *name* is gated exactly like a chain on the expression.

  - An `$op(...)` call whose result category is invariant, read from `OPERATOR_RETURNS` (`src/operators.ts`). Absence from that table means "depends on the arguments", so `$add`/`$subtract` (number **or** date), the element readers (`$min`, `$max`, `$first`, `$arrayElemAt`, …) and the pass-throughs (`$ifNull`, `$cond`, `$switch`, `$let`, `$getField`, `$reduce`, …) never seed a rejection.

  Object literals stay uncertain (a `{$op}` escape hatch can return any type), as does every unknown receiver: a field ref, a `.find()`/`.at()`/`.max()` element, or a `.concat`/`.reduce` result. Those return `null` and never throw — the literal-gating guarantee, mirroring the date-receiver gate.
- `requiredReceiverFamily(method)` — the receiver type a method **requires**, or `null` for dual/universal methods (`.slice`/`.concat`/`.indexOf`/`.includes`/`.lastIndexOf` accept string OR array; `.size` array OR object; `.toString`/`.getTime`/`.toLocaleString` any) which are therefore never gated (their legitimate multi-type uses always compile). Derived from `receiver` (`"date"`), the `NUMBER_RECEIVER_METHODS` / `OBJECT_RECEIVER_METHODS` sets, and `optional`.

  **A single-type method that declares no family is silently ungated**, which is the failure mode this pair has to be audited against — the gate can only fire for a method that says what it needs. Every array-only method therefore carries `optional: "array"`, including the ones whose lowering only ever throws: the mutator shims (`.sort`, `.push`, `.pop`, `.shift`, `.unshift`, `.splice`, `.fill`, `.copyWithin`) and the void/iterator shims (`.forEach`, `.entries`, `.keys`, `.values`). Declaring it puts the **type** error ahead of the shim's advice, which matters because "use '.toSorted()'" is wrong guidance for a string. `.difference` / `.intersection` / `.union` declare it too: a `new Set(...)` receiver is intercepted before `generateMethodCall`, so reaching the dispatch means the plain-array form. `.isSubsetOf` / `.isSupersetOf` are the exception — they have no plain-receiver form at all, so a non-Set receiver falls through to "Unknown method" and there is no family to declare.

  The complement is asserted in `test/codegen.test.ts`: the set of methods with **no** family must equal a written-out list of the dual, universal and unreachable ones. A new single-type method that forgets its family fails that assertion instead of quietly emitting MQL the server refuses.

`rejectIncompatibleChain` compares them: a `bool` receiver rejects every method except `.toString`/`.getTime` (a boolean has no others); any other type rejects a method whose required family differs. Two hints are family-specific because the conversion is the answer the user wants: a date receiver under a string method names `.format("%Y-%m-%d")` / `.toISOString()`, and under a number method names `.getTime()` and the part getters.

**The authority for these rejections is JavaScript, not mongod.** JSMQL is a strict subset of JavaScript, where `date.toUpperCase()` and `"abc".map(…)` are `TypeError`s — so they are compile errors here even where MQL would quietly coerce. Most of the rules coincide with a genuine server rejection (`$map` over a boolean/number/string/date, `$size` over a date, `$trim` over a date), but not all do: mongod **accepts** `$toUpper` on a date and returns the stringified date. Rejecting that is deliberate — a developer who writes `.toUpperCase()` on a date has a bug their editor would flag in any other JavaScript file. This is the one place the chain check is stricter than the server, and it does not weaken the escape hatch: a hand-written `$toUpper($.t)` passes through untouched, because raw MQL is the developer's own and outside this check.

The deliberately-uncertain cases (value-mode `.find(...)/.at(...)` element type unknown, `.clamp(...)` number-or-date, dual methods) still emit. In the `$$$.<coll>` lookup path an element-returning terminal followed by an array/string method is caught earlier, in `lookup-translation.ts` — a `$$$.<coll>` stream is a stream of documents, so there the element type *is* certain even though the registry can't say so. `documentReceiverViolation` is the shared predicate; it also covers the dual `string | array` methods that `requiredReceiverFamily` deliberately spares, because a document is neither. See [lookup-stage.md](lookup-stage.md).
...
| Method | MQL output |
|---|---|
| `.at(n)` | `{ $arrayElemAt: [expr, n] }` if array-producing, `{ $substrCP: [expr, <n resolved>, 1] }` if string-producing, a three-way `$cond` otherwise (see below) |
...
**Bare built-in callbacks.** All single-param lambda callbacks above also accept a bare reference to a one-value-in/one-value-out built-in in place of a `Lambda`: `Boolean`/`Number`/`String` (`TypeCastRef`), the single-argument `Math` methods (`MathCallRef`), and `ObjectId` (`ObjectIdRef`). `bareCallbackToLambda(arg, param)` in `codegen.ts` is the single desugar — it returns the synthetic `Lambda { params: [param], body: <the call applied to that param> }`, or `null` for anything else. Two value-mode entry points apply it, which is what makes the vocabulary uniform across methods:

| Entry point | Covers | Param |
|---|---|---|
| `requireLambda()` | the array methods above (`.map` / `.filter` / `.find` / …) | `"v"` |
| `resolveIteratee()` | the lodash iteratee & predicate methods (`.uniqBy` / `.keyBy` / `.reject` / …) | the iteratee gensym |

Each desugars to exactly what the equivalent arrow lowers to, so no per-method handler changes. `.reduce()` / `.reduceRight()` reject the form through their existing 2-or-3 param check (a synthetic lambda has 1 param).

It is deliberately **not** applied inside `shorthandToLambda()`, the shared shorthand hub. That hub also serves the document-stream callers (stream methods, `$lookup` predicates), whose element is a whole document — `$$.countBy(String)` would stringify a document, which the server rejects. Those callers keep their own form-list rejection instead. `parseInt`/`parseFloat` and `Date` are excluded because their real-JS bare form does not mean what it reads as; see [grammar.md](grammar.md#built-in-call-vs-bare-reference) and the `BareCastOp` comment in `ast.ts`.

**lodash iteratee/predicate shorthands.** Alongside the type-cast desugar, `requireLambda()` also desugars the three `_.identity`-family shorthands into synthetic arrows via `shorthandToLambda(arg, method, "jsmqlItem")` — a **property string** `"a.b"` → `it => it.a.b`, a **matches object** `{ a: 1, b }` → `it => it.a === 1 && it.b === b` (flat `$eq` per key; single key emits a bare `$eq`, multiple an `$and`), and a **matchesProperty pair** `["a.b", v]` → `it => it.a.b === v`. Because they become ordinary arrows before lowering, each method's own value/boolean handling applies unchanged (a predicate method's `jsBoolIfNeeded` gives a property string JS-truthy semantics; `.map` reads it as a plucked value). The lodash iteratee/predicate helpers (`resolveIteratee` → the `By` family; `resolvePredicate` → `.reject`/`.partition`/`.takeWhile`/… delegates to it) share the same `shorthandToLambda`, so the shorthand vocabulary and emitted shape are identical across native and lodash methods. `resolvePredicate` adds the `jsBoolIfNeeded` wrap its callers read as a boolean (it carries the resolved iteratee's source AST on `ResolvedIteratee.src` to make the elision check possible), which is what keeps `.reject(p)` the exact complement of `.filter(p)` and both `.partition` buckets consistent with them — under raw MQL truthiness an element whose predicate value is `""` fell out of *both* halves. The `_.matches` comparison is flat `$eq`, **not** lodash's deep partial match.
...
JS array-method callbacks receive `(element, index, array)`. jsmql supports all three. They are shared across `.map`, `.filter`, `.find`, `.findLast`, `.some`, `.every`, `.flatMap` via the `arrayIterInput()` helper in `codegen.ts` (`.findIndex`/`.findLastIndex` keep their own inline 2-param path). Those seven reach it through `LowerInput.callback()`, the service their declarations in `src/methods/array-callbacks.ts` read — see [lowering-grid.md](lowering-grid.md).

JavaScript's **fourth** argument — the trailing `thisArg` on the method itself, not on the callback — is **rejected**. A jsmql expression has no `this`, so the argument could only ever be discarded, and discarding it silently is the failure mode the arity rules exist to prevent. `requireLambda` raises it rather than a count rule, because "is there a callback here?" is one question and a count would answer it worse.
...
| `.format(spec[, opts])` | `{ $dateToString: { date: expr, format: spec[, timezone] } }` | MQL specifiers; a Moment token string is rejected |
...
`checkMonthBase` rejects a literal month below 1 at every one of those sites. Both engines *accept* it (month 0 is December of the year before), so nothing downstream would ever complain — which is the whole reason it is caught: a pasted-in JS `new Date(y, 0, d)` would silently shift a query by a year. A month above 12 passes: `new Date($.y, 13, 1)` rolls into next January in both engines, and `m + 1` arithmetic legitimately produces it. The check is literal-gated through `litNumber`, so it also catches the unary-minus form and no-ops on a runtime month.
...
**`.set` and the two part families.** `$dateFromParts` needs every part it will use, so overriding one means reading the rest back: a `$let` binds `$dateToParts` once (via `internalVar(ctx, "parts")`, so a lambda parameter spelled `jsmqlParts` gets gensymed around) and each unnamed part reads `$$jsmqlParts.<key>`. When the call overrides **all** of the family's parts there is nothing to read back, and the `$let` and `$dateToParts` are both dropped — a bare `$dateFromParts`.

The families (`DATE_PARTS_CALENDAR`, `DATE_PARTS_ISO`) are the operator's own two key sets, and mongod refuses a document that mixes them ("$dateFromParts does not allow mixing natural dates with ISO dates"), so a mix is a compile-time error naming both offending keys. The family is chosen by whichever marker set appears (`DATE_PARTS_ISO_MARKERS` / `DATE_PARTS_CALENDAR_MARKERS`); the four time parts belong to both, so a time-only call defaults to calendar, where the two are equivalent. The ISO family also needs `iso8601: true` on the `$dateToParts` read, or the read returns calendar parts the rebuild has no slot for.

Three rules make the shape unambiguous. Months are **1-based**, the base `.getMonth()` and `$month` use. `timezone` is not a part — it is the trailing options argument, and a `timezone` key inside the parts object gets a message naming the second-argument form rather than a generic unknown-key error, because it is a reasonable thing to try. And the parts object must be a written-out literal for the same reason the options object must: MongoDB reads the field names, so they have to exist in source, while the values stay free to be paths or parameters.

Part values are gated to `int-or-long`, and `$dateFromParts` carries the same `keyTypes` in the operator registry — mongod rejects a string or a fractional double in any part ("'year' must evaluate to an integer"), so both spellings reject it with the same wording rather than the method inventing a restriction of its own.

**Granular comparison.** `.isSame` / `.isBefore` / `.isAfter` build one `$dateTrunc` per side through a local `bucketOf` helper and compare with `$eq` / `$lt` / `$gt`. The unit and the options are resolved per side rather than shared, so no generated sub-object appears twice in the output tree. The options must reach **both** truncations: a timezone or `startOfWeek` applied to one side only would compare two different calendars, which is a silently wrong answer rather than an error.

The unit is mandatory, and the arity check is preceded by a dedicated one-argument rejection because the generic count message would not tell the user the real problem: `.isSame(other)` is `===`, `.isBefore(other)` is `<`, `.isAfter(other)` is `>`, all of which JSMQL already lowers, so the error names the operator to write instead. That check runs before `checkArity` so a one-argument call gets the specific message and any other count gets the standard one.
...
`momentFormatHint` builds the suggestion with a left-to-right longest-match scan over `MOMENT_FORMAT_TOKENS`, whose entries map to a specifier or to `null` where MongoDB has none (month name, weekday name, 12-hour clock, 2-digit year, ordinal day). The scan order matters: `MMM` must be consumed whole, or `MM` → `%m` leaves a stray `M` and the error names a token the user never wrote. A translation is offered only when nothing untranslated survives — tested by stripping the `%X` pairs from the result and looking for remaining letters, which also catches tokens the table doesn't list (`A`) — otherwise the message names what has no specifier and points at deriving it from the numeric parts. The translation is only ever error text; jsmql never guesses a format into output.
...
- Keys are emitted in `DATE_OPTION_ORDER` (`binSize`, `timezone`, `startOfWeek`) — the operators' own field order, not the written order. No operator carries more than three, so one total order serves the whole family.
- Per-key literal gating lives in `DATE_OPTION_CHECK` and calls the same helpers the operator path calls (`checkArgType` for `binSize`/`timezone`, `checkArgEnum(…, "weekday")` for `startOfWeek`), so both spellings produce the same message.
- A field a method's operator has but the options set deliberately omits stays reachable through that operator's `$op(...)` form — `.format` takes `timezone` only, so `$dateToString`'s `onNull` is written as `$dateToString({ date, format, onNull })`. The omission is not a capability gap, and adding the key here would have to keep `.format`'s invariant `returns: "string"` honest.

**Zero-argument accessors.** The date methods JavaScript's own `Date` already carries — the accessors plus `.getTime()` / `.toISOString()` — take no arguments: their lowering reads the receiver and nothing else, so an argument has nowhere to go. `NATIVE_DATE_METHODS` in [src/codegen.ts](../../src/codegen.ts) is the list, and `generateMethodCall` arity-checks it once before the dispatch switch rather than per `case`. Without it an extra argument vanished in silence — the one error shape a user can't spot in the emitted MQL. The methods that *do* take a timezone (`.week([timezone])`, `.quarter([timezone])`, …) keep their own `checkArity` at the `case`. The same list is what `scripts/generate-globals.mjs` skips when it augments `interface Date`, because lib.d.ts types those already.
...
`bindingTypes` holds the static type (`"object" | "array" | "string"`) of selected in-scope bindings:

- `.reduce()` accumulators (see below);
- **array-method lambda *element* parameters** typed from the input array's static element type (`arrayElementType` — fed through `elementTypedCtx` for `.map`/`.filter`/`.find`/`.findLast`/`.some`/`.every`/`.flatMap`/`.findIndex`/`.findLastIndex`, and inline for `.reduce`/`.reduceRight`);
- pipeline `const` declarations whose initialiser has a provable static type (`extendCtxLets` consults `staticBindingType`). A `const` bound to a direct `$$$.<coll>` lookup is typed by `lookupSlotType` — `.filter` / `.aggregate` leave the `$lookup.as` array in place, while `.find` stays untyped because its `$first` yields `null` on no match. A `let` is left untracked entirely: a later reassignment could change its type, so it keeps the conservative runtime dispatch;
- `$lookup.let` correlation vars that capture a whole outer binding — see [lookup-stage.md § Correlation-var types](lookup-stage.md).

It is keyed by the user-facing param name (pre-`reduceRemap`).

**Every receiver-type dispatch reads it.** `bindingTypeOf(expr, ctx)` folds the lookup into the three structural predicates (`isArrayProducing` / `isStringProducing` / `isObjectProducing`), each of which takes an optional trailing `ctx`: a binding of known type is as provable as a literal or an invariant-`returns` chain, so passing `ctx` is what lets `.includes` / `.indexOf` / `.slice` / `.concat` / `.toString` / `.size` / `.length` / `IndexAccess` / string-context `+` / template interpolation / `certainReceiverType` all reach their precise branch instead of a runtime `$cond` whose other branch can never run. The predicates answer from structure alone when `ctx` is omitted — the one caller that must (`staticBindingType`, reached from `pipeline.ts` on a raw AST) is where a *new* binding's type is being computed. The two `lambdaResult(lambda)` inference sites deliberately stay ctx-free: that expression comes from *inside* a lambda whose params shadow the outer scope, so the outer types don't apply to it.

`extendCtx` and every manual ctx-literal in `codegen.ts` forward the field; `freshSubPipelineCtx` deliberately does **not** (lambda-scoped narrowing must not cross into a sub-pipeline that runs against a different document — a `$lookup` re-seeds only the correlation-var slice), but `freshFacetCtx` **does** (the `const` slots persist into facet branches, so their types remain valid).
...
- A **provably-string key** (`isStringProducing(idx, ctx)` — a string literal, template literal, `.toLowerCase()`-style string-returning method, a binding typed `"string"`, etc.) → `$getField` directly, **even when the receiver is a known array**. A string is never a numeric array index, so the access is unambiguously an object property getter. This is not just an optimisation: `$arrayElemAt` *rejects* a string index at runtime ("second argument must be a numeric value, but is string"), so the old `$isArray`/`$arrayElemAt` dual guard would turn a valid-JS access (`arr["x"]` is a property lookup in JS, e.g. `[1,2]["length"]`) into a server error whenever the value is an array — an HR3 violation. `$getField` on an array input is accepted and yields missing, the faithful lowering.

An **integer-literal key** takes its own path, `generateNumericIndexAccess` — see § Numeric bracket keys below. Everything else (`$.cart.field[$.mainSide]`) takes the general dispatch: known-array receiver → `$arrayElemAt`; a **structurally- or binding-typed object** receiver → `$getField`; otherwise the runtime `$cond` on `$isArray`. A *structural* object is one provably never an array — the bare root document (`$` → `$$ROOT`, always a BSON object) or an object literal — so a computed key on it (`$[$.k]`, `$[SSTM_PROP[party]]`, `({a:1})[$.k]`) skips the dispatch and lowers straight to `$getField`, not a `$cond` whose dead `$arrayElemAt` branch would carry a string field name as an array index (rejected at *pipeline-optimization* time — "$arrayElemAt's second argument must be a numeric value, but is string" — on engines that don't prune unreachable branches; an HR3 violation that only surfaces on some servers). Notably there is **no** `["length"]` special case — `$.field["length"]` reads a property called "length" (now via `$getField`, since `"length"` is a string key), it does not compute a size.

### Numeric bracket keys — `generateNumericIndexAccess`

An **integer-literal** key is the one bracket shape whose three JavaScript readings each lower to a different operator, so it is handled in one place rather than falling through the array-or-object dispatch:

| Receiver | Emitted | JS |
|---|---|---|
| known array | `{ $arrayElemAt: [obj, i] }` | `["a","b"][0] === "a"` |
| known string | `{ $substrCP: [obj, i, 1] }` | `"abc"[0] === "a"` |
| known object (incl. the bare root) | `{ $getField: { field: "<i>", input: obj } }` | `({ 0: "z" })[0] === "z"` |
| unknown | `$isArray` → array, else `isStringType` → string, else field | all three |

Two hard constraints shape it, both verified against a live server:

- **`$getField.field` must evaluate to a String.** mongod refuses `field: 0` outright ("$getField requires 'field' to evaluate to type String, but got int"), and this is an *executor*-time error, so the guard's else-branch aborted the whole command for every non-array receiver. The digit is stringified at compile time — which is also what JS does, since a property key coerces to a string.
- **A negative literal is rejected for every receiver type.** `arr[-1]` is `undefined` in JavaScript; only `.at(-1)` counts from the end. `$arrayElemAt` *would* return the last element and `$substrCP` would be refused by the server, so all three candidate answers are wrong — the error names `.at()` instead. Literal-gated: a computed index (`arr[$.i]`) can't be proven either way and passes through as written.

A **provably-string** key still short-circuits to `$getField` ahead of all of this (a string is never an index), and the `["length"]` raw-access rule is unaffected — it arrives as a string key.

### Computed keys are coerced — `coerceFieldKey`

The same String requirement binds a key jsmql **can't** prove is a string (`$.doc[$.k]`, a lambda element param of unknown type). Such a key is emitted as `{ $toString: { $ifNull: [k, ""] } }`:

- `$toString` is the faithful lowering, not a workaround — a JavaScript property key is always coerced, so `obj[0]` *is* `obj["0"]`.
- The `$ifNull` is the half that matters in practice. `$toString` of a **missing** key is null, which mongod rejects the same way — so `$.doc[$.k]` aborted the whole command on every document that simply lacked `k`, ordinary data rather than an exotic case. `""` names a field no document has, so an absent key reads as missing (the sensible analogue of JS looking up the `"undefined"` property).

Provably-string keys skip it entirely (`isStringProducing`), and an integer literal is folded to its string at compile time, so the coercion only appears where the type is genuinely open.

### Compiler-owned slots are typed — `slotTypes` / `noteSlotType`

A method chained onto a materialised lookup (`$$$.users.filter(p).at(0)`) has a plain field path as its receiver — `__jsmql.tmp.<N>` — which no structural predicate can type, even though **jsmql itself filled it**. `GenerateCtx.slotTypes` closes that: the lowering records what it wrote (`noteSlotType`) and `bindingTypeOf` reads it for a `FieldRef`, so the whole dual-dispatch family resolves at compile time over the compiler's own scratch instead of guarding at runtime.

It is a **mutable** map shared by reference — the sibling of `slotAllocator`, and pipeline-global for the same reason (it describes emitted stages, not per-document state). That is what lets it work through the update-op buffer without threading a return value through `extractLookupCalls`' seventeen call sites: lowering writes it strictly before codegen reads it, because every caller runs the extraction and then generates the rewritten expression under the same ctx object.

Written only where the slot's contents are certain — a `.filter`/`.aggregate` `$lookup.as` array (`lookupSlotType`), or the single object a collapsing terminal (`.countBy`/`.keyBy`/`.groupBy("k")`) unwraps. An overwrite whose type isn't certain records nothing and keeps the conservative dispatch: `.find` (`$first` is `null` on no match), `.length` (a count), `.reduce` (the accumulator).
...
## Type-aware dispatch for `.includes` / `.indexOf` / `.at` / `.concat`

These three methods exist on both strings and arrays in JS but compile to different MQL operators. Codegen consults `isArrayProducing(expr, ctx)` and `isStringProducing(expr, ctx)` — so a *binding* of known type resolves as precisely as a literal does (§ `bindingTypes`); `.slice` / `.toString` / `.size` / `.length` dispatch through the same two predicates:
...
**`.at` (and its lodash spelling `.nth`, which shares `generateIndexFromEitherEnd`) carries two extra constraints.** It is the *only* spelling of a negative index (brackets reject one, § Numeric bracket keys), and JS has it on `String` as well as `Array` — so it sits in the `"either"` `optional` family. The two branches need **different index expressions**: `$arrayElemAt` accepts a negative index natively, while `$substrCP` refuses one outright ("the starting index must be nonnegative integer"), so the string side routes the index through `normaliseSliceIndex` (the helper `.slice`/`.substr` share). And its unknown-receiver `else` tests `isStringType` explicitly rather than treating "not an array" as "string": `$substrCP` of a missing value is `""`, which is **not null**, so the loose form poisons an enclosing `??` — `$.aliases.at(0) ?? "anonymous"` yielded `""` instead of the fallback. The third branch is `$$REMOVE` (JS has no `.at()` on a number or a document, and missing is how MQL spells an absent result).

The unknown-receiver path is intentionally verbose. When the user knows the type at design time, they can pin it by binding the value to a `const` with a provable initialiser, by chaining a type-fixing method (e.g. `.toLowerCase()` for strings, `.slice()` for arrays), or by switching to the operator form (`$in`, `$indexOfArray`, `$concatArrays`).
...
JS bracket access serves two purposes that compile to different MQL operators: array indexing (`$arrayElemAt`) and object dynamic-key lookup (`$getField`). Codegen resolves the receiver type from `isArrayProducing(expr.object, ctx)`, then structural-or-binding object detection (`isObjectProducing(expr.object, ctx)`, or the bare root — `FieldRef` with `path === ""`), and independently asks whether the **key** is provably a string (`isStringProducing(expr.index, ctx)`). Dispatch, in order:
...
- **Known array** receiver (`isArrayProducing` true — structurally or through `bindingTypes`) → `{ $arrayElemAt: [obj, idx] }`. Optional-chain fallback: `[]`.
...
The key-is-string check beats the receiver type because a string key can only mean property access (never an array index), and the runtime guard would otherwise emit a server-rejected `$arrayElemAt`-with-string shape. An *integer-literal* index has no such shortcut: all three receiver readings are live, so it routes to `generateNumericIndexAccess` (§ Numeric bracket keys). If a user wants compact output for `$.field[0]`, they pin the receiver type — bind it to a `const` with a type-revealing initialiser, or chain `.map(x => x)` / `.slice(0)` / `.reverse()`. (`.at(0)` is not a shortcut: it is dual-type too, and dispatches on an unknown receiver just as brackets do.) Inside a `.reduce()` body, the accumulator parameter is narrowed automatically when initialValue and body agree on a compound type — see the reduce parameter-remapping section above.
