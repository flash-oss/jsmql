...
- `certainReceiverType(object, ctx)` — the receiver's provable, invariant type, or `null` when uncertain. `bool`/`array` reuse the verified-sound `isProvablyBool` / `isArrayProducing` (which recurse through `.slice`/`MethodCall` and subsume literals + array-typed operators); `string`/`number`/`object` are read from the receiver **method's** invariant `returns`, or from `ctx.bindingTypes` when the receiver is a typed binding (§ `bindingTypes`) — so `const s = $.name.trim(); s.map(f)` is caught at compile time — deliberately **not** from `isStringProducing` (its `STRING_OUTPUT_OPS` wrongly holds the int-returning `$strcasecmp`) nor object literals (a `{$op}` escape hatch can return any type). Every unknown receiver (a field ref, a `.find()`/`.at()`/`.max()` element of unknown type, a `.concat`/operator/`{$op}`-literal result) returns `null` and never throws — the literal-gating guarantee, mirroring the date-receiver gate.
