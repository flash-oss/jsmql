...
- `certainReceiverType(object)` — the receiver's provable, invariant type, or `null` when uncertain. It reports every family the compiler can be sure of:
  - `bool` / `array` reuse the verified-sound `isProvablyBool` / `isArrayProducing`, which recurse through `.slice`/`MethodCall` and subsume literals **and array-/bool-returning operator calls** (so `$split(…).trim()` and `$eq(…).map(…)` are already caught).
  - `string` / `number` / `object` come from the receiver **method's** invariant `returns` — deliberately **not** from `isStringProducing`, whose `STRING_OUTPUT_OPS` wrongly holds the int-returning `$strcasecmp`.
  ...
  Object literals stay uncertain (a `{$op}` escape hatch can return any type), as does every unknown receiver: a field ref, a `.find()`/`.at()`/`.max()` element, a `.concat`/`.reduce` result, or a string/number/date/object-returning `$op(...)` call. Those return `null` and never throw — the literal-gating guarantee, mirroring the date-receiver gate.
