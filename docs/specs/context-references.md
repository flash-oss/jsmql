...
- **Ambient globals** — **partially shipped** `[DEF-015]`. `src/globals.ts` now declares `$$`, `$$$`, `$$$$` as ambient `const`s so the arrow-form syntax type-checks under TypeScript. The collection-scoped (`$$`) and cluster-scoped (`$$$$`) diagnostic source stages are typed precisely (annotated option objects + JSDoc), and the `$$` stream vocabulary (`.filter`, `.map`, `.slice`, `.concat`, `.toSorted`, `.toReversed`, `.flatMap`, `.push`) is typed for completion. Every remaining ref form (`.coll.find(...)`, `$out`, `$$ = …`, member access) rides a permissive `[key: string]: any` tail — and even the typed members return `any` (completion, not real document typing). Narrowing those to real collection/document types is still future work, gated on schema threading (DEF-013). See `globals-generation.md` § Context references and DEF-015.
