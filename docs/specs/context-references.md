...
Tests use the string form rather than the arrow form, but the arrow form now type-checks too: `$$` / `$$$` / `$$$$` are declared as ambient globals in [`src/ops.ts`](../../src/ops.ts) (via `import "@koresar/jsmql/ops"`), with completion for the diagnostic source stages — see `ops-generation.md` § Context references.
...
- **Ambient globals** — **partially shipped** `[DEF-015]`. `src/ops.ts` now declares `$$`, `$$$`, `$$$$` as ambient `const`s so the arrow-form syntax type-checks under TypeScript. The collection-scoped (`$$`) and cluster-scoped (`$$$$`) diagnostic source stages are typed precisely (annotated option objects + JSDoc); every other ref form (`.push`, `.filter`, `.coll.find(...)`, `$out`, stream methods, member access) rides a permissive `[key: string]: any` tail. Narrowing those to real collection/document types is still future work, gated on schema threading (DEF-013). See `ops-generation.md` § Context references and DEF-015.
