...
- **Ambient globals** — `src/globals.ts` declares `$$` / `$$$` / `$$$$` so the arrow-form syntax type-checks, and types the surface each one carries: the diagnostic source stages with annotated option objects, and — for the collection ref and the foreign-collection ref `$$$.<coll>` indexes to — the stream vocabulary, the chained stage calls, the stream count, and the value terminals. Chainable members return their own ref, so a chain keeps the identity of its root. What stays untyped is the *document*: values are `any`, because per-collection document typing needs schema threading (DEF-013). See `globals-generation.md` § Context references.
