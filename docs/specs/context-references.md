...
Tests use the string form rather than the arrow form, but the arrow form now type-checks too: `$$` / `$$$` / `$$$$` are declared as ambient globals in [`src/ops.ts`](../../src/ops.ts) (via `import "@koresar/jsmql/ops"`), with completion for the diagnostic source stages and — on `$$` — the stream vocabulary (`$$.filter(...)`, `$$.map(...)`, `$$.slice(...)`, …) — see `ops-generation.md` § Context references.
...
- **`$$$$.<db>.<coll>.find / .filter(...)` cross-database read → `$lookup` with `from: { db, coll }`** — **rejected** (not supported). A cross-database read can only compile to a `{ db, coll }` `$lookup`/`$unionWith` namespace, which is Atlas-Data-Federation-only — every standalone / replica-set / sharded MongoDB rejects it at runtime — so jsmql rejects it at compile time (HR3). The rejection (and the `$$.push(...$$$$...)` union / `$ = $$$$...` replace-root / `$$ = $$$$...` source-switch variants) fires at the shared `requireSameDbColl` choke point with a message matching `/Cross-database reads aren't supported/` that redirects to same-database `$$$.<coll>`. See [`lookup-stage.md`](./lookup-stage.md) → "Cross-database reads are rejected" and [LANGUAGE.md → Cross-database reads](../LANGUAGE.md#cross-database-reads-not-supported). The cross-database **write** (`$$$$.<db>.<coll> = $$` → `$out`) is supported — see the next entry.
...
- **Diagnostic source stages — direct call on a bare ref (`$$.indexStats()`, `$$$$.currentOp(...)`, `$$$$.shardedDataDistribution()`)** — **shipped**. A *direct* `MethodCall` on a bare `CollectionRef` / `ClusterRef` (distinct from the lookup's `MemberAccess`-wrapped receiver) lowers to MongoDB's diagnostic / system source stages. The ref prefix encodes the scope MongoDB requires — two tiers: collection (`$$`, run on `db.coll.aggregate()`) and cluster/server (`$$$$`, run on the admin DB). `$$$` (current database) has no diagnostics — `$currentOp` & friends run on admin, not the current DB. A wrong-scope use is a compile-time error. First-stage-only; Pipeline-mode-only. The scope ↔ stage mapping lives in the `diagnostic` field of the STAGES registry. See [`system-stages.md`](./system-stages.md) and [LANGUAGE.md → System / diagnostic stages](../LANGUAGE.md#system--diagnostic-stages-indexstats-currentop-).
...
- **Ambient globals** — **partially shipped** `[DEF-015]`. `src/ops.ts` now declares `$$`, `$$$`, `$$$$` as ambient `const`s so the arrow-form syntax type-checks under TypeScript. The collection-scoped (`$$`) and cluster-scoped (`$$$$`) diagnostic source stages are typed precisely (annotated option objects + JSDoc), and the `$$` stream vocabulary (`.filter`, `.map`, `.slice`, `.concat`, `.toSorted`, `.toReversed`, `.flatMap`, `.push`) is typed for completion. Every remaining ref form (`.coll.find(...)`, `$out`, `$$ = …`, member access) rides a permissive `[key: string]: any` tail — and even the typed members return `any` (completion, not real document typing). Narrowing those to real collection/document types is still future work, gated on schema threading (DEF-013). See `ops-generation.md` § Context references and DEF-015.
- **Nested lookups** — `$$$.coll2.find/filter(...)` (or any `$$$$` variant) inside another lookup's predicate or `.aggregate` block. Currently rejected with a targeted error by `rejectNestedLookup` in [`lookup-stage.md`](./lookup-stage.md).
