...
One stage is exempt from the codegen's default string-literal protection: **`$unwind`**. By default a string literal that begins with `$` is wrapped in `{ $literal: … }` (so `"$items"` in an *expression* stays the literal six-character string rather than a field reference — see `literalSafeString` in [src/codegen.ts](../../src/codegen.ts)). But `$unwind`'s body is a **field-path position**, not an expression: `$unwind("$items")` and `$unwind({ path: "$items", … })` both mean "unwind the field `items`", so the leading `$` is exactly what the user intends. `generateStageBody` therefore lowers the `$unwind` body with `ctx.fieldPathString = true`, which suppresses the `$literal` wrap so the path passes through raw (`{ $unwind: "$items" }`). The `$`-prefix itself is still required and is enforced by `validateUnwind` ([src/stage-validation.ts](../../src/stage-validation.ts)); `$unwind("items")` is rejected. This exemption is `$unwind`-only — stages that take a string in expression position (e.g. `$sortByCount("$items")`) keep the protection, because there a literal string vs a field path is the user's choice between `"$x"` and `$.x`.
