...
- **`typeof` speaks MongoDB's type names, and nothing else.** `typeof $.a === "bool"` is the test; `"boolean"`, `"function"` and every other spelling MongoDB's `$type` does not know is refused with the nearest name, never lowered to a test that quietly matches nothing. `"undefined"` IS a MongoDB type — the deprecated BSON one — and means it; absence has its own spelling, `$.a === undefined`. The list is `BSON_TYPE_ALIASES` in `src/registry/vocabulary.ts`, and `"number"` is the one umbrella (the expression form compares against the concrete names it covers, `TYPE_GROUPS`).
