...
**Spec.** `docs/specs/mutations.md` covers the AST, lexer additions, parser dispatch, codegen coalescer, and pipeline integration. User-facing reference is `docs/LANGUAGE.md` § Mutations. Tests in `test/mutations.test.ts` (56 cases) plus a realistic end-to-end case in `test/realistic.test.ts`.
