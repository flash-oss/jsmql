...
All 27 instance-method checks migrated; output is unchanged (only message text, which was almost entirely unasserted) and the full suite stays green, plus three new tests lock in the exact/range/atLeast/none message shapes. The static families (`Math.`/`Object.`/`Set.`/`regex.`) keep their current wording for now — `checkArity` takes a `prefix` param so they can adopt it later; `Math` already centralizes its 1-arg check via `oneArg`. Spec: [method-dispatch.md](specs/method-dispatch.md). The per-method return-type sets (`STRING_RETURNING_METHODS`, …) and the lowering bodies themselves stay in place — folding those into a data registry is possible but offers little over the named switch, so it wasn't pursued here.
