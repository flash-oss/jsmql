...
## 2026-05-24 — `this.<collection>.find/filter(predicate)` → `$lookup`

`$lookup` was the one major MongoDB stage with no JS-style spelling — users still had to drop to the raw `$lookup({ from, localField, foreignField, as })` callable for the most common join shape in the wild. The new syntax fills that gap: `$.user = this.users.find(u => u._id === $.userId);` lowers to `$lookup` + `$unwind(preserve)`, and `$.orders = this.orders.filter(o => o.userId === $._id);` lowers to `$lookup` (array stays). Spec: [docs/specs/lookup-stage.md](specs/lookup-stage.md). User-facing reference: [docs/LANGUAGE.md → Cross-collection lookups](LANGUAGE.md#cross-collection-lookups-thiscollection).

**Why `this.` and not `super.`.** `super.users.find(...)` was semantically perfect — "escape this scope outward" — and was the first proposal. It's a hard JS syntax blocker: `super` is parse-rejected outside class/method bodies, so the no-`;` arrow form `($) => super.users.find(...)` would refuse to round-trip through a `.js` file. That violates the strippable-JS rule in the root `CLAUDE.md` (priority #2: every accepted construct must parse as JS when pasted into a file). `this.<coll>` parses anywhere and survives the round trip, and `this` cleanly carries "the implicit thing this pipeline is bound to" semantics — close enough to `super` for the metaphor and within the syntax envelope. The keyword is recognised in `keywordToken` ([src/lexer.ts](../src/lexer.ts)) and gates a new `ThisRef` AST node ([src/ast.ts](../src/ast.ts)).

**Why `.find` ↔ `$unwind(preserve)` and `.filter` ↔ bare `$lookup`.** JS's `Array.prototype.find` returns one element or `undefined`; `Array.prototype.filter` returns an array. The same single/many distinction maps cleanly to SQL's scalar-subquery / LEFT-JOIN split, and the JS user already knows which to reach for. Lowering: `.find` always emits the two-stage `$lookup` + `$unwind(preserveNullAndEmptyArrays: true)` so the row is preserved on no match and the field is the matched doc (not an array) on a match. The acknowledged caveat is multi-match: if the predicate matches >1 foreign doc, `$unwind` fans the row out. Basic-form `$lookup` can't `LIMIT 1`; that requires pipeline-form `$lookup`. The caveat is documented in the LANGUAGE reference rather than worked around, because the alternative (`$lookup` + `$set: { x: { $first: "$x" } }`) lies about the case where there's a single match but extracts a value at the cost of one extra stage on every join — pessimising the common case to make the rare multi-match case behave more like JS. We picked transparency over the smaller-than-it-looks gain.

**Why v1 is basic-form only.** MongoDB's basic-form `$lookup` (`{ from, localField, foreignField, as }`) is *intrinsically* limited to two field-path strings — anything richer (computed values, additional conjuncts, ranges) requires the pipeline form (`let` + `pipeline: [{ $match: $expr: … }]`). v1's predicate translator (`translateLookupEquality` in [src/lookup-translation.ts](../src/lookup-translation.ts)) accepts a single equality between two field paths and rejects everything else with an actionable error pointing at the explicit `$lookup({ from, let, pipeline, as })` form. Auto-generating the pipeline form with `let` extraction is future work in the spec; shipping it in v1 would have doubled the surface area and the open-design questions (how to identify which `$.x` refs to lift into `let`, whether to inline simple cases, what to do about side-effecting expressions on RHS) for a feature most users won't need on day one.

**The grammar `this.<coll>.<method>()` is reserved, methods are not invented.** v1 ships `.find` and `.filter`; the spec claims the cross-collection vocabulary for future MongoDB stages so we won't have to relitigate it. But any future method on `this.<coll>` must map to an **existing JS API** — no invented names. The defensible candidate today is `Array.prototype.concat` for `$unionWith` (`this.events.concat($ => …)` reads as "this followed by the argument"). `$graphLookup`, `$merge`, `$out` have no obvious existing-JS-API analog and naming is explicitly deferred. Better to leave the slot empty than to lock users into vocabulary they can't transfer.

**Interception, not codegen-special-casing.** The lookup detection lives in [src/pipeline.ts](../src/pipeline.ts) (`emitUpdateOpsWithLookups`), called from both `generateImplicitPipeline` (the `;`-separated form) and `generatePipeline` (the bracketed form). Inside, the AssignExpr buffer is split at every `this.<coll>.find/filter(...)` op: pending non-lookup ops flush as `$set`/`$unset`, then the `$lookup` (+ `$unwind` for find) emits inline, then buffering resumes. No new code paths in `codegen.ts` aside from the user-facing `ThisRef` error case for misuse — `$lookup` is fundamentally a stage, not an expression, so it correctly stays in pipeline.ts's lane. The `ThisRef` error message names `;`-terminated Pipeline statements explicitly so a Filter-mode misuse points at the right fix in one read.
