...
| `specs/system-stages.md` | `$$.indexStats()` / `$$$.currentOp(…)` / `$$$$.shardedDataDistribution()` → diagnostic / system *source* stage lowering: the scope ↔ stage mapping in the STAGES `diagnostic` field, detection of a direct ref `MethodCall` (and disambiguation from `$lookup`'s `MemberAccess`-wrapped receiver), the `$$`-namespace sharing with `.push`/`.filter`, first-stage-only enforcement, the wrong-scope / unknown-method / arg-shape error catalog, and the Pipeline-mode-only gate. |
