...
| `specs/stream-methods.md` | Registry of chainable JS-array-shaped methods on the RHS of `$$ = …` — currently `.slice(start, end?)`. Each entry declares an arg-shape validator and a `lower(args, ctx, callPos) → { stages, clearLets? }`; the chain walker in `pipeline.ts` (`lowerChainOnStream` for `$$` heads, `lowerChainOnCollection` for `$$$.<coll>` heads) reads from it. Adding a method is a registry-entry + tests change, no parser or chain-walker edits. |
