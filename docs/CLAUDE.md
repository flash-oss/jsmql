...
| `specs/lookup-stage.md` | The `this.<coll>.find/filter(predicate)` syntax for `$lookup`: `ThisRef` AST node, pipeline-mode-only invariant, find/filter lowering rules, the v1 basic-form scope, and the reserved cross-collection grammar for future stages. |
