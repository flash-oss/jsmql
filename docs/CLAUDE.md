...
| `specs/replace-root-stage.md` | `$ = <expr>` → `$replaceWith` lowering: bare-`$` as a primary expression (lowers to `"$$ROOT"`), `AssignExpr`-with-empty-path-target interception, the direct-lookup-RHS leaner form (skips the `$set $first` stage), the four compile-time RHS rejections (array, scalar literal, `.filter()` lookup, compound desugar), the bare-`$` `delete`-target rejection, and the let-scope-clearing thread through `lowerUpdateFilterWithLookups`. |
