...
| `specs/ops-generation.md` | The `jsmql/ops` subpath: how `src/ops.ts` is generated at build time from `OPERATORS` + `STAGES` + the vendored MQL spec, the type-mapping rules, and the drift test. |
