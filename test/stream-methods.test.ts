...
describe(".slice — rejection branches", () => {
  it("zero args → 1-or-2 args error", () => {
    expect(() => jsmql("$$ = $$.slice();")).toThrow("'.slice(start[, end])' requires 1 or 2 arguments, got 0");
  });

  it("three args → 1-or-2 args error", () => {
    expect(() => jsmql("$$ = $$.slice(0, 5, 10);")).toThrow("'.slice(start[, end])' requires 1 or 2 arguments, got 3");
  });
  ...
  it("fractional start → non-negative integer error", () => {
    expect(() => jsmql("$$ = $$.slice(1.5, 5);")).toThrow("'slice' expects an integer, but got a number.");
  });

  it("end < start → ordered-bounds error", () => {
    expect(jsmql("$$ = $$.slice(10, 5);")).toEqual([{ $skip: 10 }, { $match: { $expr: false } }]);
  });
...
describe("'from the end' array methods are NOT on the stream surface", () => {
  ...
  it("the rejection names the take-from-the-front rewrite", () => {
    expect(() => jsmql("$$ = $$.takeRight(3);")).toThrow(
...
describe("stream .takeWhile / .dropWhile → $setWindowFields running flag", () => {
  ...
  it("lifts sortBy from ANY sort spelling", () => {
    ...
    expect(sortBy("$$.sortBy(d => d.cat.toLowerCase()).takeWhile(o => o.ok);")).toEqual({ "__jsmql.tmp.0": 1 });
  });
  ...
  it("with no preceding sort it REJECTS — never defaults to _id", () => {
    ...
    for (const m of ["takeWhile", "dropWhile"]) {
      expect(() => jsmql(`$$.${m}(o => o.ok);`), m).toThrow(/Sort first/);
...
describe(".sort(<sort>) / .toSorted(<sort>) → $sort — flexible sort args", () => {
  ...
  it(".toSorted is equivalent to .sort on a stream", () => {
    expect(jsmql('$$ = $$.toSorted("createdAt");')).toEqual([{ $sort: { createdAt: 1 } }]);
  ...
  it(".orderBy(keys, orders) → $sort with per-key directions (fewer orders default asc)", () => {
    ...
    expect(() => jsmql('$$ = $$.orderBy("age", "up");')).toThrow(
      '.orderBy(keys, orders) takes 1, -1, "asc" or "desc" for each order.',
    );
  });
...
describe(".reject(pred) → $match (filter negated)", () => {
  ...
  it("rejects a spread / multi-arg", () => {
    expect(() => jsmql("$$.reject(o => o.a, o => o.b);")).toThrow(
...
describe(".map(d => <expr>) — chain-form per-doc reshape", () => {
  ...
  it("zero args is rejected", () => {
    expect(() => jsmql("$$ = $$.map();")).toThrow("'.map(callback)' requires exactly 1 argument, got 0");
  });
  ...
  it("`$.<field>` inside .map body is rejected — must use the lambda param", () => {
    expect(jsmql("$$ = $$.map(d => ({ n: $.name }));")).toEqual([{ $replaceWith: { n: "$name" } }]);
  });
      ...
      { $match: { $expr: false } },
      {
        $unionWith: {
          ...
          pipeline: [
            ...
            { $replaceWith: { archives: "$__jsmql.tmp.0" } },
            { $unset: "__jsmql" },
          ],
        },
      },
...
describe(".toSorted((a, b) => …) — comparator → $sort", () => {
  ...
  it("non-comparator body is rejected", () => {
    expect(() => jsmql("$$ = $$.toSorted((a, b) => a.x + b.x);")).toThrow(
      ".toSorted((a, b) => …) compares one field of each: 'a.age - b.age', or 'b.age - a.age' for descending. Join keys with '||'.",
    );
  });

  it("mismatched paths in subtraction are rejected", () => {
    expect(() => jsmql("$$ = $$.toSorted((a, b) => a.x - b.y);")).toThrow(
...
describe("$$ = [{ key: $$.reduce(…) }] wrap — JS-faithful fold-to-summary via $group + $replaceWith", () => {
  ...
  it("multiple keyed accumulators share one $group", () => {
    expect(
      ...
      { $replaceWith: { count: "$count", total: "$total", best: { $max: [0, "$best"] } } },
    ]);
  });
  ...
  it("zero args inside the wrap is rejected with the reduce-arity error", () => {
    expect(() => jsmql("$$ = [{ total: $$.reduce() }];")).toThrow(
      "'$$.reduce(reducer, init)' takes the reducer and its initial value — 2 arguments, got 0.",
    );
  });
...
describe("$$ = [$$.reduce((acc, d) => ({...acc, …}), {…})] — object-returning reducer wrap", () => {
  ...
  it("non-object init is rejected with a hint", () => {
    expect(() => jsmql("$$ = [$$.reduce((acc, d) => ({ ...acc, count: acc.count + 1 }), 0)];")).toThrow(
      "'count' is uneven between the reducer and its init — the body folds it, the init does not start it. Name every accumulator in both.",
    );
  });
  ...
  it("body and init keys must match — extra body key is rejected", () => {
    expect(() =>
      ...
      "'total' is uneven between the reducer and its init — the body folds it, the init does not start it. Name every accumulator in both.",
    );
  });
  ...
  it("body and init keys must match — extra init key is rejected", () => {
    expect(() => jsmql("$$ = [$$.reduce((acc, d) => ({ count: acc.count + 1 }), { count: 0, total: 0 })];")).toThrow(
      "'total' is uneven between the reducer and its init — the init starts it, the body does not fold it. Name every accumulator in both.",
    );
  });
  ...
  it("entry referencing acc.<otherKey> instead of acc.<sameKey> is rejected", () => {
    ...
    expect(() =>
      ...
      "This reducer body spells no MongoDB accumulator. The shapes that do: 'acc.total + d.<field>' ($sum), 'acc.total + 1' ($sum: 1), 'Math.max(acc.total, d.<field>)' / 'Math.min(…)' ($max / $min), 'acc.total ?? d.<field>' ($first), 'd.<field>' ($last), '[...acc.total, d.<field>]' or 'acc.total.concat(d.<field>)' ($push). For anything else write '$group({ … })'.",
    );
  });
...
describe("$$ = [$$.reduce((acc, d) => ({...acc, [d.<k>]: <v>}), {})] — dict-build reducer wrap", () => {
  ...
  it("optional leading `...acc` spread", () => {
    ...
    expect(() => jsmql("$$ = [$$.reduce((acc, d) => ({ [d.id]: d.name }), {})];")).toThrow(
  ...
  it("rejects non-empty init via the object-reducer's missing-keys check", () => {
    ...
    expect(() => jsmql("$$ = [$$.reduce((acc, d) => ({ ...acc, [d.id]: d.name }), { fallback: null })];")).toThrow(
      "A fold keyed by a field — '({ ...acc, [d.k]: d.v })' — starts from '{}'.",
    );
  });
});
...
describe("unknown chain method on $$ → registry error with hint", () => {
  ...
  it("unknown method after a valid .filter still surfaces the registry list", () => {
    expect(() => jsmql("$$ = $$.filter(o => o.active).bogus();")).toThrow(
      "'.bogus()' is not a method of the stream '$$'. A stage is a link too: '$$.$match(…)'.",
    );
  });
});
...
describe("stream callbacks — spelling never changes the emitted MQL", () => {
  ...
  for (const spelling of PREDICATE_SPELLINGS) {
    ...
    it(`.map(${spelling}) emits exactly what the equivalent arrow does`, () => {
      ...
      expect(() => jsmql(`$.o = $$$.orders.map(${spelling});`)).toThrow(/takes/);
    });
  }
  ...
  it("a computed key lowers straight into $group._id — no extra stages", () => {
    ...
    expect(jsmql(`$$ = $$.keyBy({ active: true });`)).toEqual([
      { $group: { _id: { $eq: ["$active", true] }, __jsmqlTmp: { $last: "$$ROOT" } } },
      {
        $group: {
          _id: null,
          __jsmqlTmp: { $push: { k: { $ifNull: [{ $toString: "$_id" }, "null"] }, v: "$__jsmqlTmp" } },
    ...
    expect(jsmql(`$$ = $$.countBy(["status", "open"]);`)).toEqual([
      { $group: { _id: { $eq: ["$status", "open"] }, __jsmqlTmp: { $sum: 1 } } },
      {
        $group: {
          _id: null,
          __jsmqlTmp: { $push: { k: { $ifNull: [{ $toString: "$_id" }, "null"] }, v: "$__jsmqlTmp" } },
  ...
  it("an object on a sort method still means directions, not a matcher", () => {
    expect(jsmql(`$.o = $$$.orders.orderBy({ cat: -1 });`)).toEqual([
  ...
  it("a computed-key iteratee's own errors name the calling method, not `.map`", () => {
    ...
    expect(jsmql(`$$ = $$.countBy(d => $.other);`)).toEqual([
      { $group: { _id: "$other", __jsmqlTmp: { $sum: 1 } } },
      {
        $group: {
          _id: null,
          __jsmqlTmp: { $push: { k: { $ifNull: [{ $toString: "$_id" }, "null"] }, v: "$__jsmqlTmp" } },
    ...
    expect(jsmql(`$$ = $$.uniqBy(function (d) { const y = d.x; return y; });`)).toEqual([
      { $group: { _id: { $let: { vars: { y: "$x" }, in: "$$y" } }, __jsmqlTmp: { $first: "$$ROOT" } } },
      { $replaceWith: "$__jsmqlTmp" },
    ]);
    expect(jsmql(`$$ = $$.countBy((d, i) => d.x);`)).toEqual([
      { $group: { _id: "$x", __jsmqlTmp: { $sum: 1 } } },
      {
        $group: {
          _id: null,
          __jsmqlTmp: { $push: { k: { $ifNull: [{ $toString: "$_id" }, "null"] }, v: "$__jsmqlTmp" } },
