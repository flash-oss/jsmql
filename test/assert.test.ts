    ...
    expect(jsmql("assert($.qty >= 0, 'qty bad'); $.fee = $.qty * 0.01")).toEqual([
      {
        $match: {
          $expr: {
            $convert: {
              input: true,
              to: { $cond: [{ $gte: ["$qty", 0] }, "bool", "jsmql assertion failed: qty bad"] },
            },
          },
        },
      },
              ...
              to: {
                $cond: [
                  ...
                  "bool",
                  {
                    $concat: [
                      "jsmql assertion failed: ",
                      { $toString: { $concat: ["qty was ", { $toString: "$qty" }] } },
                    ],
                  },
                ],
              },
...
describe("assert — call forms", () => {
    ...
    expect(jsmql("[assert($.q >= 0, 'm'), $sort({ q: 1 })]")).toEqual([
      {
        $match: {
          $expr: {
            $convert: { input: true, to: { $cond: [{ $gte: ["$q", 0] }, "bool", "jsmql assertion failed: m"] } },
          },
        },
      },
      { $sort: { q: 1 } },
  ...
  it("is accepted by jsmql.pipeline()", () => {
    expect(jsmql.pipeline("assert($.q >= 0, 'm')")).toEqual([
      {
        $match: {
          $expr: {
            $convert: { input: true, to: { $cond: [{ $gte: ["$q", 0] }, "bool", "jsmql assertion failed: m"] } },
...
describe("assert — rejections", () => {
  ...
  it("rejects a spread argument", () => {
    expect(() => jsmql("assert(...$.flags)")).toThrow(
      "Spread (...) is not supported in assert(...) — pass operands directly (assert(a, b)) or as a single array (assert([a, b])).",
    );
  });
