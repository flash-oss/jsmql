...
describe("operator literal-type validation — date slots", () => {
  ...
  it("allows a field ref, a $-string field path, and new Date(...) in a date slot (gate)", () => {
    ...
    expect(jsmql.expr('$dateAdd({ startDate: new Date("2020-01-01"), unit: "day", amount: 1 })')).toEqual({
      $dateAdd: { startDate: new Date("2020-01-01T00:00:00.000Z"), unit: "day", amount: 1 },
    });
...
describe("jsmql template-tag form", () => {
  ...
  describe("opaque BSON value interpolation", () => {
    ...
    it("RegExp interpolation lands in query-doc form as a real RegExp", () => {
      ...
      expect(out.username).toEqual(/^alice/i);
    });
    ...
    it("multiple Dates inside one object each get their own binding", () => {
      ...
      expect(out.since).toEqual(new Date("2026-01-01T00:00:00.000Z"));
      expect(out.until).toEqual(new Date("2026-02-01T00:00:00.000Z"));
    });
    ...
    it("Date inside an interpolated array preserves position and instance", () => {
      ...
      expect(out[0]).toEqual(new Date("2026-01-01T00:00:00.000Z"));
      expect(out[1]).toEqual(new Date("2026-02-01T00:00:00.000Z"));
    });
    ...
    it("mixed JSON + Date inside one interpolated object: each leaf keeps its shape", () => {
      ...
      expect(out).toEqual({ name: "foo", since: new Date("2026-01-01T00:00:00.000Z"), count: 42 });
      expect(out.since).toEqual(new Date("2026-01-01T00:00:00.000Z"));
    });
    ...
    it("deeply-nested opaque value (3 levels) survives", () => {
      ...
      expect(out.a.b.c).toEqual(new Date("2026-01-01T00:00:00.000Z"));
    });
    ...
    it("nested RegExp inside an interpolated object preserves the instance", () => {
      ...
      expect(out.pat).toEqual(/^alice/i);
    ...
    it("$dateDiff with an interpolated object carrying Date instances — realistic shape", () => {
      ...
      expect(out.$dateDiff.startDate).toEqual(new Date("2026-01-01T00:00:00.000Z"));
      expect(out.$dateDiff.endDate).toEqual(new Date("2026-02-01T00:00:00.000Z"));
...
describe("ObjectId literal (in-source constant)", () => {
  ...
  it('works as an object-literal value too — { _id: ObjectId("…") } and the new form', () => {
    expect(jsmql(`{ _id: ObjectId("${HEX}") }`)).toEqual({ _id: new ObjectId("698a76556c10b90d8bd0497e") });
    expect(jsmql(`{ _id: new ObjectId("${HEX}") }`)).toEqual({ _id: new ObjectId("698a76556c10b90d8bd0497e") });
  });
  ...
  it("ObjectId literal in aggregation-expression position lands directly in the expression", () => {
    ...
    expect(out.$eq[1].toHexString()).toBe("698a76556c10b90d8bd0497e");
  });
...
describe("ObjectId from a 0x hex literal", () => {
  ...
  it("a 0x literal as an object-literal value mints an ObjectId (the canonical _id lookup)", () => {
    expect(jsmql(`{ _id: 0x${HEX} }`)).toEqual({ _id: new ObjectId("507f1f77bcf86cd799439011") });
  });

  it("0x literals inside a raw filter doc's $in array each mint an ObjectId", () => {
    expect(jsmql(`{ _id: { $in: [0x${HEX}, 0x${HEX2}] } }`)).toEqual({
      _id: { $in: [new ObjectId("507f1f77bcf86cd799439011"), new ObjectId("698a76556c10b90d8bd0497e")] },
    });
  });
...
describe("jsmql.compile — opaque BSON bindings outside query-doc position", () => {
  ...
  it("Date binding lands as a real Date inside an update op", () => {
    ...
    expect(out[0].$set.lastSeenAt).toEqual(new Date("2026-01-01T00:00:00.000Z"));
  });
  ...
  it("RegExp binding lands as a real RegExp inside an update op", () => {
    ...
    expect(out[0].$set.name).toEqual(/^alice/i);
  });
  ...
  it("Date nested inside a binding object preserves the instance", () => {
    ...
    expect(out[0].$set.window.since).toEqual(new Date("2026-01-01T00:00:00.000Z"));
    expect(out[0].$set.window.until).toEqual(new Date("2026-02-01T00:00:00.000Z"));
  ...
  it("Date inside a binding array preserves each instance", () => {
    ...
    expect(out[0].$set.bounds[0]).toEqual(new Date("2026-01-01T00:00:00.000Z"));
    expect(out[0].$set.bounds[1]).toEqual(new Date("2026-02-01T00:00:00.000Z"));
  });
  ...
  it("deeply-nested (3 levels) Date inside a binding survives", () => {
    ...
    expect(out[0].$set.cfg.a.b.c).toEqual(new Date("2026-01-01T00:00:00.000Z"));
  });
  ...
  it("mixed JSON + Date inside a binding object: each leaf keeps its shape", () => {
    ...
    expect(out[0].$set.cfg).toEqual({
      name: "foo",
      since: new Date("2026-01-01T00:00:00.000Z"),
      count: 42,
      until: new Date("2026-02-01T00:00:00.000Z"),
    });
    expect(out[0].$set.cfg.since).toEqual(new Date("2026-01-01T00:00:00.000Z"));
    expect(out[0].$set.cfg.until).toEqual(new Date("2026-02-01T00:00:00.000Z"));
  });
  ...
  it("nested RegExp inside a binding object preserves the instance", () => {
    ...
    expect(out[0].$set.matchers.name).toEqual(/^alice/i);
    expect(out[0].$set.matchers.status).toEqual(/^ok$/);
  });
  ...
  it("nested Uint8Array inside a binding object preserves the instance", () => {
    ...
    expect(out[0].$set.payloads.hash).toEqual(new Uint8Array([1, 2, 3]));
  });
  ...
  it("nested ObjectId (duck-typed) inside a binding object preserves the instance", () => {
    ...
    expect(out[0].$set.ids.primary).toEqual({ _bsontype: "ObjectID", id: "abc" });
    expect(out[0].$set.ids.secondary).toEqual({ _bsontype: "ObjectId", id: "xyz" });
  });
  ...
  it("Date inside a binding lands in $set body alongside JSON-shaped siblings (realistic shape)", () => {
    ...
    expect(out[0].$set.cfg.startedAt).toEqual(new Date("2026-01-01T00:00:00.000Z"));
...
describe("jsmql.validate()", () => {
  ...
  it("returns error for trailing junk", () => {
    ...
    expect(result.valid).toBe(false);
    expect(result.errors[0].message).toMatch("Expected ';' but got 'garbage' at position 15");
  });
});
...
describe("arithmetic operators", () => {
  ...
  it("** is right-associative", () => {
    expect(jsmql.expr("2 ** 3 ** 2")).toEqual(512);
  });
});
...
describe("logical operators", () => {
  ...
  it("! unary uses JS truthiness", () => {
    expect(jsmql.expr("!$.active")).toEqual({
      $not: {
        $and: [
...
describe("ternary", () => {
  ...
  it("ternary with non-bool condition wraps in jsBool", () => {
    expect(jsmql.expr('$.name ? "yes" : "no"')).toEqual({
      $cond: {
        if: {
          $and: [
            { $ne: [{ $ifNull: ["$name", null] }, null] },
            { $ne: ["$name", false] },
            { $ne: ["$name", ""] },
            { $ne: ["$name", 0] },
          ],
...
describe("bracket access", () => {
  ...
  it("a numeric object KEY builds the stringified field name, as JavaScript does", () => {
    ...
    expect(jsmql.expr("({ 0: 1, 1.5: 2, 0x10: 3 })")).toEqual({ "0": 1, "16": 3, "1.5": 2 });
  ...
  it("object-literal elements type the element as object → element[k] → $getField", () => {
    expect(jsmql.expr("[{ a: 1 }, { b: 2 }].map(o => o[$.k])")).toEqual({
      $map: {
        input: [{ a: 1 }, { b: 2 }],
        as: "o",
        in: { $getField: { field: { $toString: { $ifNull: ["$k", ""] } }, input: "$$o" } },
      },
    });
  });
...
describe("field path regression (FieldRef stops at first segment)", () => {
  ...
  it("rejects numeric field segments — $.items.0 is not valid JS syntax", () => {
    expect(() => jsmql.expr("$.items.0")).toThrow("Expected a name but got '0' at position 8");
  });
...
describe("string methods", () => {
  ...
  it("toLowerCase", () => {
    expect(jsmql.expr("$.name.toLowerCase()")).toEqual({
      $cond: { if: { $eq: [{ $ifNull: ["$name", null] }, null] }, then: null, else: { $toLower: "$name" } },
    });
  });
  it("toUpperCase", () => {
    expect(jsmql.expr("$.name.toUpperCase()")).toEqual({
      $cond: { if: { $eq: [{ $ifNull: ["$name", null] }, null] }, then: null, else: { $toUpper: "$name" } },
    });
  });
  it("substr", () => {
    expect(jsmql.expr("$.name.substr(0, 5)")).toEqual({
      $cond: { if: { $eq: [{ $ifNull: ["$name", null] }, null] }, then: null, else: { $substrCP: ["$name", 0, 5] } },
    });
  });
  it("split", () => {
  ...
  it("indexOf on known string → $indexOfCP", () => {
    expect(jsmql.expr('$.name.toLowerCase().indexOf("@")')).toEqual({
      $indexOfCP: [
        { $cond: { if: { $eq: [{ $ifNull: ["$name", null] }, null] }, then: null, else: { $toLower: "$name" } } },
        "@",
      ],
  ...
  it("match with regex literal", () => {
    expect(jsmql.expr("$.str.match(/^[A-Z]/)")).toEqual({
      $cond: {
        if: { $eq: [{ $ifNull: ["$str", null] }, null] },
        then: null,
        else: { $regexMatch: { input: "$str", regex: "^[A-Z]" } },
      },
    });
  });
  it("match with regex literal and flags", () => {
    expect(jsmql.expr("$.str.match(/^[a-z]/i)")).toEqual({
      $cond: {
        if: { $eq: [{ $ifNull: ["$str", null] }, null] },
        then: null,
        else: { $regexMatch: { input: "$str", regex: "^[a-z]", options: "i" } },
      },
    });
  });
  ...
  it("chained toLowerCase then trim", () => {
    expect(jsmql.expr("$.name.toLowerCase().trim()")).toEqual({
      $trim: {
        input: {
          $cond: { if: { $eq: [{ $ifNull: ["$name", null] }, null] }, then: null, else: { $toLower: "$name" } },
        },
...
describe("typeof: the Query and Expr targets agree", () => {
  ...
  it("maps JavaScript's spelling to the BSON alias in BOTH targets", () => {
    ...
    expect(() => jsmql.expr('typeof $.a === "boolean"')).toThrow(
      "'typeof' compares against one of MongoDB's type names, and \"boolean\" is not one. Did you mean 'bool'? For absence, write 'x === undefined'.",
    );
  });
...
describe("method arg-count errors (one formatter over the row's `args`)", () => {
  ...
  it("range counts read '1 or 2' / '0, 1, or 2'", () => {
    ...
    expect(() => jsmql.expr("$.s.padStart()")).toThrow(
      "'.padStart(targetLength[, padString])' requires 1 or 2 arguments, got 0",
    );
  });
  ...
  it("a callback method rejects JavaScript's trailing thisArg instead of dropping it", () => {
    ...
    expect(() => jsmql.expr("$.a.map(5)")).toThrow(
...
describe("array methods (no lambda)", () => {
  ...
  it("slice(start, end) on known array → count is end - start (JS end-exclusive)", () => {
    ...
    expect(jsmql.expr("[1,2,3,4,5].slice(1, 3)")).toEqual([2, 3]);
  });
  ...
  it("slice(-n) → last n elements (the 2-arg $slice primitive)", () => {
    expect(jsmql.expr("[1,2,3,4,5].slice(-2)")).toEqual([4, 5]);
  });
    ...
    expect(setOf(p, 2)).toEqual({
      inc: {
        $cond: {
          if: { $eq: [{ $ifNull: ["$__jsmql.var.s", null] }, null] },
          then: null,
          else: { $gte: [{ $indexOfCP: ["$__jsmql.var.s", "x"] }, 0] },
        },
      },
      ...
      sl: {
        $cond: {
          if: { $eq: [{ $ifNull: ["$__jsmql.var.s", null] }, null] },
          then: null,
          else: { $substrCP: ["$__jsmql.var.s", 1, 2] },
        },
      },
...
describe("bare built-in callbacks", () => {
  ...
  it("reduce(Boolean, 0) hits the existing param-count error", () => {
    expect(() => jsmql.expr("$.xs.reduce(Boolean, 0)")).toThrow(
...
describe("date arithmetic (.plus / .minus)", () => {
  ...
  it("rejects a non-integer literal amount (mongod requires an integer)", () => {
    expect(() => jsmql.expr('$.t.plus(1.5, "day")')).toThrow("'plus' expects an integer, but got a number.");
    expect(() => jsmql.expr('$.t.minus("30", "day")')).toThrow("'minus' expects an integer, but got a string.");
  });
  it("rejects a non-string literal timezone", () => {
    expect(() => jsmql.expr('$.t.plus(1, "day", 5)')).toThrow(
      "'plus' takes a string or a document here, and a number is neither.",
    );
  });
...
describe("replacing date parts (.set)", () => {
  ...
  it("rejects an unknown part with a suggestion", () => {
    expect(() => jsmql.expr("$.t.set({ yaer: 2030 })")).toThrow(
      "'set' has no parameter 'yaer'. Did you mean 'year'? Valid keys: year, month, day, isoWeekYear, isoWeek, isoDayOfWeek, hour, minute, second, millisecond.",
    );
  });
  it("points a timezone inside the parts object at the second argument", () => {
    expect(() => jsmql.expr('$.t.set({ timezone: "UTC" })')).toThrow(
      "'set' has no parameter 'timezone'. Valid keys: year, month, day, isoWeekYear, isoWeek, isoDayOfWeek, hour, minute, second, millisecond.",
    );
  });
...
describe("date parts JavaScript has no getter for", () => {
  ...
  it("rejects extra arguments, unknown options and a literal non-date receiver", () => {
    ...
    expect(() => jsmql.expr("$.t.quarter({ binSize: 2 })")).toThrow(
      "'quarter' has no parameter 'binSize'. Valid keys: timezone.",
    );
    expect(() => jsmql.expr('"x".week()')).toThrow(
      "'.week()' is not available on a 'string' — it is defined on 'date'.",
    );
  });
});
...
describe("date formatting (.format)", () => {
  ...
  it("rejects a specifier MongoDB has no format character for", () => {
    expect(() => jsmql.expr('$.createdAt.format("%Q")')).toThrow(
      "'format' format has an invalid specifier '%Q'. MongoDB accepts %Y %G %m %d %j %U %V %u %w %H %M %S %L %z %Z and %%.",
    );
  });
  ...
  it("rejects a non-string literal format and the wrong argument count", () => {
    expect(() => jsmql.expr("$.createdAt.format(5)")).toThrow("'format' expects a string, but got a number.");
  ...
  it("takes timezone only — $dateToString's onNull stays on the operator form", () => {
    expect(() => jsmql.expr('$.createdAt.format("%Y", { onNull: "" })')).toThrow(
      "'format' has no parameter 'onNull'. Valid keys: timezone.",
    );
  });
});
...
describe("date truncation (.startOf)", () => {
  ...
  it("rejects a literal non-date receiver", () => {
    expect(() => jsmql.expr('"2020-01-01".startOf("day")')).toThrow(
      "'.startOf()' is not available on a 'string' — it is defined on 'date'.",
    );
  });
});
...
describe("date difference (.diff)", () => {
  ...
  it("rejects a literal non-date argument", () => {
    expect(() => jsmql.expr('$.end.diff("2020-01-01", "day")')).toThrow(
      "'diff' expects a date, but got a string. Use a field path or new Date(…).",
    );
  });
...
describe("date-method trailing options argument", () => {
  it("rejects an unknown option with a suggestion and the valid set", () => {
    expect(() => jsmql.expr('$.end.diff($.start, "day", { startofweek: "monday" })')).toThrow(
      "'diff' has no parameter 'startofweek'. Did you mean 'startOfWeek'? Valid keys: timezone, startOfWeek.",
    );
  });
  it("rejects an option the method's operator has no field for", () => {
    expect(() => jsmql.expr('$.t.plus(1, "day", { binSize: 2 })')).toThrow(
      "'plus' has no parameter 'binSize'. Valid keys: timezone.",
    );
  });
...
describe("date-method receiver type-check", () => {
  it("rejects a literal non-date receiver, consistent with the operator form", () => {
    expect(() => jsmql.expr('"2020-01-01".getFullYear()')).toThrow(
      "'.getFullYear()' is not available on a 'string' — it is defined on 'date'.",
    );
    expect(() => jsmql.expr('"2020".toISOString()')).toThrow(
      "'.toISOString()' is not available on a 'string' — it is defined on 'date'.",
    );
    expect(() => jsmql.expr('"2020".getUTCHours()')).toThrow(
      "'.getUTCHours()' is not available on a 'string' — it is defined on 'date'.",
    );
    expect(() => jsmql.expr('"2020-01-01".plus(1, "day")')).toThrow(
      "'.plus()' is not available on a 'string' — it is defined on 'date'.",
    );
  });
  it("allows a literal non-date receiver on .getTime() — $toLong converts strings/numbers", () => {
    expect(() => jsmql.expr('"2020".getTime()')).toThrow(
      "'.getTime()' is not available on a 'string' — it is defined on 'date'.",
    );
  });
...
describe("date methods (UTC variants)", () => {
  ...
  it("getUTCMinutes", () => {
    expect(jsmql.expr("$.ts.getUTCMinutes()")).toEqual({ $minute: "$ts" });
  });
  it("getUTCSeconds", () => {
    expect(jsmql.expr("$.ts.getUTCSeconds()")).toEqual({ $second: "$ts" });
  });
  it("getUTCMilliseconds", () => {
    expect(jsmql.expr("$.ts.getUTCMilliseconds()")).toEqual({ $millisecond: "$ts" });
  });
});
...
describe("new Date()", () => {
  ...
  it("new Date(y, m, d) folds to a UTC Date", () => {
    expect(jsmql.expr("new Date(2024, 1, 15)")).toEqual(new Date("2024-02-15T00:00:00.000Z"));
  });
  it("new Date(y, m, d, h, mi, s, ms) folds to a UTC Date", () => {
    expect(jsmql.expr("new Date(2024, 12, 31, 23, 59, 58, 999)")).toEqual(new Date("2025-01-31T23:59:58.999Z"));
  });
  ...
  it("rejects more than 7 args", () => {
    expect(() => jsmql.expr("new Date(1, 2, 3, 4, 5, 6, 7, 8)")).toThrow(
      "'Date(year, month, day, hour, minute, second, ms)' requires 2, 3, 4, 5, 6, or 7 arguments, got 8",
    );
  });
});
...
describe("Date.UTC()", () => {
  ...
  it("Date.UTC(y) — year-only form", () => {
    expect(jsmql.expr("Date.UTC(1970)")).toEqual(0);
  });
  it("new Date(Date.UTC(...)) with constant parts folds to a real UTC Date", () => {
    expect(jsmql.expr("new Date(Date.UTC(2024, 1, 15))")).toEqual(new Date("2024-02-15T00:00:00.000Z"));
  });
  it("Date.UTC requires at least 1 arg", () => {
    expect(() => jsmql.expr("Date.UTC()")).toThrow(
      "'Date.UTC(year[, month, day, hour, minute, second, ms])' requires 1, 2, 3, 4, 5, 6, or 7 arguments, got 0",
    );
  });
  it("Date.UTC rejects more than 7 args", () => {
    expect(() => jsmql.expr("Date.UTC(1,2,3,4,5,6,7,8)")).toThrow(
      "'Date.UTC(year[, month, day, hour, minute, second, ms])' requires 1, 2, 3, 4, 5, 6, or 7 arguments, got 8",
    );
  });
});
...
describe("type casts", () => {
  ...
  it("Boolean() uses JS truthy semantics (not MQL's $toBool)", () => {
    expect(jsmql.expr("Boolean($.x)")).toEqual({
      $and: [
        { $ne: [{ $ifNull: ["$x", null] }, null] },
        { $ne: ["$x", false] },
        { $ne: ["$x", ""] },
...
describe("Math.* as bare callable in array methods", () => {
  ...
  it("rejects binary Math methods as bare (Math.pow)", () => {
    expect(() => jsmql.expr("$.scores.map(Math.pow)")).toThrow(
  ...
  describe("a stage-call block outside a sub-pipeline position names the real mistake", () => {
    it("a misspelled block method on a stream receiver suggests the intended one", () => {
      for (const [typo, meant] of [
        ...
        expect(() => jsmql(err)).toThrow(/Unknown method '\.\w+\(\)'/);
        expect(() => jsmql(err)).toThrow(new RegExp(`Did you mean '\\.${meant}\\(\\)'`));
      }
    });
    ...
    it("states the callback rule rather than promising the suggested method takes stages", () => {
      for (const src of [
        ...
        expect(() => jsmql(src)).toThrow(/is a pipeline stage, not part of a callback|Unknown method/);
      }
    });
    ...
    it("an in-document array receiver is told stages need a stream", () => {
      for (const src of [
        ...
        expect(() => jsmql(src)).toThrow(
          /is a pipeline stage, not part of a callback|Unknown method|produces pipeline stages/,
        );
      }
    });
    ...
    it("carries a .pos pointing at the stage call", () => {
      ...
      expect(res.errors[0].pos).toBe(16);
    });
...
describe("array method additions", () => {
  ...
  it(".toString() on a known string is a no-op", () => {
    expect(jsmql.expr("$.name.toLowerCase().toString()")).toEqual({
      $let: {
        vars: {
          jsmqlV: {
            $cond: { if: { $eq: [{ $ifNull: ["$name", null] }, null] }, then: null, else: { $toLower: "$name" } },
  ...
  it(".toString() on unknown type lowers to $toString", () => {
    expect(jsmql.expr("$.n.toString()")).toEqual({
      $let: {
        vars: { jsmqlV: "$n" },
        in: {
          $cond: {
...
describe("array callbacks support (element, index)", () => {
  ...
  it(".reduce with 4 params throws", () => {
    expect(() => jsmql.expr("$.xs.reduce((acc, x, i, arr) => acc, 0)")).toThrow(
      "'.reduce((acc, x[, i]) => …, seed)' takes a two- or three-parameter arrow with an expression body.",
    );
  });
});
...
describe("lodash object methods (per-doc value vocabulary)", () => {
  ...
  it(".pick([keys]) → object select ($getField; missing keys drop out)", () => {
    ...
    expect(() => jsmql.expr('$.o.pick("a")')).toThrow("'pick' expects an array, but got a string.");
  });
...
describe("lodash array methods (per-doc value vocabulary)", () => {
  ...
  it("countBy/groupBy/keyBy reject more than one argument (arity names the optional iteratee)", () => {
    expect(() => jsmql.expr('$.a.countBy("x", "y")')).toThrow(
      "'.countBy([iteratee])' requires 0 or 1 arguments, got 2 — JavaScript's trailing 'thisArg' has no meaning in MQL; drop it",
    );
    expect(() => jsmql.expr('$.a.groupBy("x", "y")')).toThrow(
      "'.groupBy([iteratee])' requires 0 or 1 arguments, got 2 — JavaScript's trailing 'thisArg' has no meaning in MQL; drop it",
    );
    expect(() => jsmql.expr('$.a.keyBy("x", "y")')).toThrow(
      "'.keyBy([iteratee])' requires 0 or 1 arguments, got 2 — JavaScript's trailing 'thisArg' has no meaning in MQL; drop it",
    );
  });
});
...
describe("chain type-check — reject a method on a provably-incompatible receiver", () => {
  ...
  it("rejects a string/number/object-map receiver + array method", () => {
    expect(() => jsmql.expr("$.name.toUpperCase().map(x => x)")).toThrow(
      "'.map()' is not available on a 'string' — it is defined on 'array', 'stream'.",
    );
    expect(() => jsmql.expr("$.items.size().map(x => x)")).toThrow(
  ...
  it("rejects a date-receiver + string/number/array/object method", () => {
    ...
    expect(() => jsmql.expr('$.t.startOf("month").map(x => x)')).toThrow(
      "'.map()' is not available on a 'date' — it is defined on 'array', 'stream'.",
    );
    expect(() => jsmql.expr('$.t.endOf("month").mapValues(v => v)')).toThrow(
      "'.mapValues()' is not available on a 'date' — it is defined on 'object'.",
    );
    expect(() => jsmql.expr("$.t.set({ year: 2030 }).round(2)")).toThrow(
      "'.round()' is not available on a 'date' — it is defined on 'number', 'Math'.",
    );
    expect(() => jsmql.expr('$.t.minus(1, "day").split(",")')).toThrow(
      "'.split()' is not available on a 'date' — it is defined on 'string'. Render the date as a string first: '.format(\"%Y-%m-%d\")' or '.toISOString()'.",
    );
    expect(() => jsmql.expr("new Date($.x).map(y => y)")).toThrow(
      "'.map()' is not available on a 'date' — it is defined on 'array', 'stream'.",
    );
  });
  ...
  it("rejects a wrong-family method on a literal or template receiver", () => {
    ...
    expect(() => jsmql.expr('"abc".mapValues(v => v)')).toThrow(
      "'.mapValues()' is not available on a 'string' — it is defined on 'object'.",
    );
    expect(() => jsmql.expr("(5).trim()")).toThrow(
      "'.trim()' is not available on a 'number' — it is defined on 'string'. Render the number as a string first: '.toString()'.",
    );
    expect(() => jsmql.expr("(5).map(x => x)")).toThrow(
      "'.map()' is not available on a 'number' — it is defined on 'array', 'stream'.",
    );
    expect(() => jsmql.expr("`a${$.x}b`.plus(1, 'day')")).toThrow(
      "'.plus()' is not available on a 'string' — it is defined on 'date'.",
    );
    expect(() => jsmql.expr("(typeof $.x).map(c => c)")).toThrow(
      "'.map()' is not available on a 'string' — it is defined on 'array', 'stream'.",
    );
  });
  ...
  it("rejects the array-only methods on a non-array receiver", () => {
    ...
    expect(() => jsmql.expr("$.s.trim().toSpliced(0, 1)")).toThrow(
      "'.toSpliced()' is not available on a 'string' — it is defined on 'array'.",
    );
    expect(() => jsmql.expr("$.s.trim().with(0, 1)")).toThrow(
      "'.with()' is not available on a 'string' — it is defined on 'array'.",
    );
    expect(() => jsmql.expr("$.n.round(2).reduceRight((a, b) => a, 0)")).toThrow(
      "'.reduceRight()' is not available on a 'number' — it is defined on 'array'.",
    );
    expect(() => jsmql.expr('$.t.startOf("month").findIndex(x => x)')).toThrow(
      "'.findIndex()' is not available on a 'date' — it is defined on 'array'.",
    );
...
describe("lodash transpose value methods — zip / unzip / zipWith", () => {
  ...
  it(".zipWith(...arrays, fn) applies an N-parameter arrow (one param per array) to each group", () => {
    ...
    expect(() => jsmql.expr("$.a.zipWith($.b, x => x)")).toThrow(
  ...
  it(".truncate() default (length 30, '...'); .truncate({ length }) overrides", () => {
    ...
    expect(jsmql.expr("$.s.truncate()")).toEqual({
      $let: {
        ...
        in: {
          $cond: [
            { $gt: [{ $strLenCP: "$$jsmqlStr" }, 30] },
            { $concat: [{ $substrCP: ["$$jsmqlStr", 0, 27] }, "..."] },
            "$$jsmqlStr",
          ],
        },
      },
    ...
    expect(jsmql.expr("$.s.truncate({ length: 10 })")).toEqual({
      $let: {
        ...
        in: {
          $cond: [
            { $gt: [{ $strLenCP: "$$jsmqlStr" }, 10] },
            { $concat: [{ $substrCP: ["$$jsmqlStr", 0, 7] }, "..."] },
            "$$jsmqlStr",
          ],
        },
      },
    });
    expect(() => jsmql.expr('$.s.truncate({ separator: " " })')).toThrow(
      "'truncate' has no parameter 'separator'. Valid keys: length, omission.",
    );
  });
...
describe("statement-position Object.assign — mutating merge on a field path", () => {
  ...
  it("coalesces with adjacent field assignments inside a [...] pipeline literal", () => {
    expect(jsmql("[ $.x = 1, Object.assign($.profile, { a: $.foo }), $.y = 2 ]")).toEqual([
      { $set: { x: 1 } },
      { $set: { profile: { $mergeObjects: ["$profile", { a: "$foo" }] } } },
      { $set: { y: 2 } },
    ]);
  });
  ...
  it("an object-literal target (discarded result) throws an actionable error", () => {
    expect(() => jsmql("Object.assign({}, { a: 1 });")).toThrow(
  ...
  it("str.search(/re/) returns idx with -1 fallback", () => {
    expect(jsmql.expr("$.s.search(/foo/)")).toEqual({
      $cond: {
        if: { $eq: [{ $ifNull: ["$s", null] }, null] },
        then: null,
        else: { $ifNull: [{ $getField: { field: "idx", input: { $regexFind: { input: "$s", regex: "foo" } } } }, -1] },
      },
    });
  });
});
...
describe("Number static predicates", () => {
  ...
  it("Number.isNaN(x)", () => {
    expect(jsmql.expr("Number.isNaN($.x)")).toEqual({
      $and: [{ $isNumber: "$x" }, { $eq: [{ $toString: "$x" }, "NaN"] }],
    });
  });
...
describe("IIFE → $let", () => {
  it("simple ((x) => body)(value)", () => {
    expect(jsmql.expr("((x) => x + 1)(5)")).toEqual(6);
  });
  ...
  it("zero-param IIFE", () => {
    expect(jsmql.expr("(() => $.x + $.y)()")).toEqual({ $add: ["$x", "$y"] });
  });
  ...
  it("regex with multiple flags", () => {
    expect(jsmql.expr("$.str.match(/pattern/gi)")).toEqual({
      $cond: {
        if: { $eq: [{ $ifNull: ["$str", null] }, null] },
        then: null,
        else: { $regexMatch: { input: "$str", regex: "pattern", options: "i" } },
      },
    });
  });
});
...
describe("1-arg substr", () => {
  ...
  it("substr(start, count) keeps 2-arg form", () => {
    expect(jsmql.expr("$.name.substr(0, 3)")).toEqual({
      $cond: { if: { $eq: [{ $ifNull: ["$name", null] }, null] }, then: null, else: { $substrCP: ["$name", 0, 3] } },
    });
  });
  ...
  it("substr with a negative count yields an empty string, like JS", () => {
    expect(jsmql.expr("$.email.substr(0, -1)")).toEqual({
      $cond: { if: { $eq: [{ $ifNull: ["$email", null] }, null] }, then: null, else: { $substrCP: ["$email", 0, 0] } },
    });
  });
});
...
describe(".substring", () => {
  it("substring(start, end) folds end - start as a length", () => {
    expect(jsmql.expr("$.name.substring(2, 7)")).toEqual({
      $cond: { if: { $eq: [{ $ifNull: ["$name", null] }, null] }, then: null, else: { $substrCP: ["$name", 2, 5] } },
    });
  });
  ...
  it("substring with non-literal start clamps to 0 through $max", () => {
    expect(jsmql.expr("$.s.substring($.i, 10)")).toEqual({
      $cond: {
        if: { $eq: [{ $ifNull: ["$s", null] }, null] },
        then: null,
        else: { $substrCP: ["$s", { $max: [0, "$i"] }, { $max: [0, { $subtract: [10, { $max: [0, "$i"] }] }] }] },
      },
    });
  });
  it("substring with negative literal clamps at compile time", () => {
    expect(jsmql.expr("$.name.substring(-3, 4)")).toEqual({
      $cond: { if: { $eq: [{ $ifNull: ["$name", null] }, null] }, then: null, else: { $substrCP: ["$name", 0, 4] } },
    });
  });
});
...
describe("template literals", () => {
  ...
  it("string-producing interpolations skip the $toString wrap", () => {
    ...
    expect(jsmql.expr("`name=${$.name.toLowerCase()}`")).toEqual({
      $concat: [
        "name=",
        { $cond: { if: { $eq: [{ $ifNull: ["$name", null] }, null] }, then: null, else: { $toLower: "$name" } } },
      ],
...
describe("array .has()", () => {
  ...
  it(".indexOf() with a predicate points at .findIndex()", () => {
    expect(() => jsmql.expr("$.items.indexOf(it => it.qty > 5)")).toThrow(
      "'.indexOf()' searches for a VALUE, not by a function. To test elements against a predicate write '.findIndex(x => …)'.",
    );
  });
});
...
describe("Object.fromEntries", () => {
  ...
  it("from array literal of pairs", () => {
    expect(jsmql.expr('Object.fromEntries([["a", 1], ["b", 2]])')).toEqual({ a: 1, b: 2 });
  });
});
...
describe("optional chaining (?.)", () => {
  ...
  it("simple optional member access", () => {
    expect(jsmql.expr("$.a?.b")).toEqual({ $ifNull: ["$a.b", null] });
  });
  it("chained optional access", () => {
    expect(jsmql.expr("$.a?.b?.c")).toEqual({ $ifNull: ["$a.b.c", null] });
  });
  ...
  it("Object.keys on optional wraps argument with {}", () => {
    expect(jsmql.expr("Object.keys($.user?.profile)")).toEqual({
      $map: {
        input: { $objectToArray: { $ifNull: [{ $ifNull: ["$user.profile", null] }, {}] } },
        as: "jsmqlKv",
        in: "$$jsmqlKv.k",
      },
    });
  ...
  it("object spread of optional is NOT wrapped ($mergeObjects ignores null)", () => {
    expect(jsmql.expr("({...$.user?.profile, name: 'x'})")).toEqual({
      $mergeObjects: [{ $ifNull: ["$user.profile", null] }, { name: "x" }],
    });
  });
  it("comparison against optional is NOT wrapped", () => {
    expect(jsmql.expr("$.user?.role === 'admin'")).toEqual({ $eq: [{ $ifNull: ["$user.role", null] }, "admin"] });
  });
  it("`==` null check against optional is NOT wrapped", () => {
    expect(jsmql.expr("$.user?.role == null")).toEqual({
      $in: [{ $type: { $ifNull: ["$user.role", null] } }, ["null", "missing"]],
    });
  });
  it("numeric arithmetic against optional is NOT wrapped (honest null > 0)", () => {
    expect(jsmql.expr("$.base + $.user?.bonus")).toEqual({ $add: ["$base", { $ifNull: ["$user.bonus", null] }] });
  });
...
describe(".startsWith / .endsWith", () => {
  it("startsWith maps to indexOf == 0", () => {
    expect(jsmql.expr('$.email.startsWith("admin")')).toEqual({
      $cond: {
        if: { $eq: [{ $ifNull: ["$email", null] }, null] },
        then: null,
        else: { $eq: [{ $indexOfCP: ["$email", "admin"] }, 0] },
      },
    });
  });
...
describe(".charAt", () => {
  it("charAt(i)", () => {
    expect(jsmql.expr("$.name.charAt(2)")).toEqual({
      $cond: { if: { $eq: [{ $ifNull: ["$name", null] }, null] }, then: null, else: { $substrCP: ["$name", 2, 1] } },
    });
  });
  ...
  it("charAt with a runtime index guards the negative case", () => {
    expect(jsmql.expr("$.name.charAt($.i)")).toEqual({
      $cond: {
        ...
        then: null,
        else: { $cond: { if: { $lt: ["$i", 0] }, then: "", else: { $substrCP: ["$name", "$i", 1] } } },
      },
    });
  });
});
...
describe("array .indexOf", () => {
  ...
  it("on known string → $indexOfCP", () => {
    expect(jsmql.expr('$.email.toLowerCase().indexOf("@")')).toEqual({
      $indexOfCP: [
        { $cond: { if: { $eq: [{ $ifNull: ["$email", null] }, null] }, then: null, else: { $toLower: "$email" } } },
        "@",
      ],
...
describe("jsmql guards a $size / $in / callback input only where the array may be missing", () => {
  ...
  it("counts, searches and iterates a value that is certainly there, as it is", () => {
    ...
    expect(jsmql.expr("Object.keys($).some(k => k === 'a')")).toEqual({
      $anyElementTrue: {
        $map: {
          input: { $map: { input: { $objectToArray: "$$ROOT" }, as: "jsmqlKv", in: "$$jsmqlKv.k" } },
          as: "k",
          in: { $eq: ["$$k", "a"] },
        },
      },
  ...
  it("a $lookup always writes its array, so a count or a search over it needs no guard", () => {
    ...
    expect(jsmql("$.has = $$$.orders.filter({ userId: $._id }).some(o => o.total > 5);")).toEqual([
      { $lookup: { from: "orders", localField: "_id", foreignField: "userId", as: "__jsmql.tmp.0" } },
      {
        $set: {
          has: { $anyElementTrue: { $map: { input: "$__jsmql.tmp.0", as: "o", in: { $gt: ["$$o.total", 5] } } } },
        },
      },
      { $unset: "__jsmql" },
    ]);
...
describe("date .getTime / .toISOString", () => {
  ...
  it("toISOString", () => {
    expect(jsmql.expr("$.ts.toISOString()")).toEqual({ $dateToString: { date: "$ts" } });
  });
});
...
describe("Math.sign / log2 / log10 / hypot / cbrt / random / constants", () => {
  ...
  it("Math.cbrt", () => {
    expect(jsmql.expr("Math.cbrt($.x)")).toEqual({
      $multiply: [{ $cmp: ["$x", 0] }, { $pow: [{ $abs: "$x" }, { $divide: [1, 3] }] }],
    });
  });
  it("Math.hypot 2-arg", () => {
  ...
  it("Math.PI", () => {
    expect(jsmql.expr("Math.PI")).toEqual(3.141592653589793);
  });
  it("Math.E", () => {
    expect(jsmql.expr("Math.E")).toEqual(2.718281828459045);
  });
});
...
describe("comments", () => {
  ...
  it("keeps // inside a regex literal as pattern", () => {
    ...
    expect(jsmql.expr("$.path.match(/[/\\\\]/)")).toEqual({
      $cond: {
        if: { $eq: [{ $ifNull: ["$path", null] }, null] },
        then: null,
        else: { $regexMatch: { input: "$path", regex: "[/\\\\]" } },
      },
    });
  });
  it("regex disambiguation works after a comment (non-value-ending)", () => {
    ...
    expect(jsmql.expr("$.path.match(/* skip */ /foo/i)")).toEqual({
      $cond: {
        if: { $eq: [{ $ifNull: ["$path", null] }, null] },
        then: null,
        else: { $regexMatch: { input: "$path", regex: "foo", options: "i" } },
      },
    });
  });
  it("divide disambiguation works after a comment (value-ending)", () => {
    ...
    expect(jsmql.expr("10 /* skip */ / 2")).toEqual(5);
  });
  it("unclosed /* throws LexError", () => {
    expect(() => jsmql.expr("$.a /* unclosed")).toThrow("Unterminated block comment at position 4");
  });
});
...
describe("function overload", () => {
  ...
  it("jsmql.validate() reports the augmented hint for closure refs", () => {
    ...
    expect(r.errors[0]?.message).toMatch("Unknown identifier 'minAge'. Did you mean '$.minAge'?");
  });
...
describe("jsmql.compile()", () => {
  ...
  describe("$match index-friendly translation (a75eb35)", () => {
    ...
    it("RegExp binding inlines as a query-doc regex match", () => {
      ...
      const name = /^alice/i;
      expect(q({ name })).toEqual({ username: /^alice/i });
    });
  });
      ...
      expect(q({ region: "AU" })).toEqual([
        {
          $lookup: {
            ...
            foreignField: "userId",
            as: "addrs",
            pipeline: [{ $match: { $expr: { $eq: ["$$jsmql_f0_country", "AU"] } } }],
            let: { jsmql_f0_country: "$country" },
          },
        },
      ]);
    });
  ...
  describe("error: missing binding at call time", () => {
    it("throws UnknownIdentifierError naming the missing key", () => {
      ...
      expect(msg).toMatch(/is a parameter of this query|Unknown identifier 'foo'/);
    });
  ...
  describe("error: malformed destructure patterns", () => {
    it("rejects a nested destructure", () => {
      ...
      expect(() => jsmql.compile(new Function("return " + src)() as never)).toThrow(
        "Expected a name but got '{' at position 6",
      );
    });
...
describe("context-reference prefixes ($$, $$$, $$$$)", () => {
  ...
  describe("$$ — current collection", () => {
    ...
    it("bracket-expr form (compile-form param) throws when the compiled function is called", () => {
      ...
      expect(() => q({ name: "users" })).toThrow(/A pipeline statement writes something|is the stream/);
    });
    ...
    it("wrong method on $$ surfaces the stream-method registry error, noting .push", () => {
      ...
      expect(() => jsmql("$$.pop({a:1})")).toThrow(
        "'.pop()' is not available on a 'stream' — it is defined on 'array'.",
      );
    });
  });
  ...
  describe("parser sanity-guards", () => {
    ...
    it("$$foo (ident with no . or [) → actionable ParseError", () => {
      const r = jsmql.validate("$$foo");
      expect(r.valid).toBe(false);
      expect(r.errors[0].message).toMatch(/got 'foo'/);
    });
    it("$$$ alone → message names '$$$' specifically", () => {
      const r = jsmql.validate("$$$");
      expect(r.valid).toBe(false);
      expect(r.errors[0].message).toMatch(/'\$\$\$' is the current database/);
    });
    it("$$$$ alone → message names '$$$$' specifically", () => {
      const r = jsmql.validate("$$$$");
      expect(r.valid).toBe(false);
      expect(r.errors[0].message).toMatch(/'\$\$\$\$' is the cluster/);
    });
  });
...
describe("trailing commas (JS syntax)", () => {
  it("Math.method args", () => {
    expect(jsmql.expr("Math.max(1, 2,)")).toEqual(2);
  });
  ...
  it("$op positional args", () => {
    expect(jsmql.expr("$add(1, 2,)")).toEqual({ $add: [1, 2] });
  });

  it("$op single-object arg stays object-style", () => {
    expect(jsmql.expr('$dateTrunc({ date: $.t, unit: "week", },)')).toEqual({
      $dateTrunc: { date: "$t", unit: "week" },
    });
  });

  it("type cast (single arg) tolerates a lone trailing comma", () => {
    expect(jsmql.expr("Number($.x,)")).toEqual({ $toDouble: "$x" });
  });

  it("type cast still rejects a real second argument", () => {
    const r = jsmql.validate("Number($.x, $.y)");
    expect(r.valid).toBe(false);
    expect(r.errors[0].message).toMatch("'Number(x)' takes exactly one value.");
  });

  it("Number static method args", () => {
    expect(jsmql.expr("Number.isInteger($.x,)")).toEqual({
      $and: [
        { $and: [{ $isNumber: "$x" }, { $not: [{ $in: [{ $toString: "$x" }, ["NaN", "Infinity", "-Infinity"]] }] }] },
        { $eq: ["$x", { $trunc: "$x" }] },
      ],
    });
  });
  ...
  it("function declaration parameter list (pipeline)", () => {
    expect(jsmql("function dbl(x,) { return x * 2 } $ = { y: dbl($.n) }")).toEqual([
      { $replaceWith: { y: { $let: { vars: { x: "$n" }, in: { $multiply: ["$$x", 2] } } } } },
    ]);
  });
...
describe("rejects fractional counts, and never passes them to $slice", () => {
  ...
  for (const [name, src] of rejected) {
    it(`rejects a fraction — ${name}`, () => {
      expect(() => jsmql.expr(src)).toThrow(/needs a whole number|expects an integer/);
    });
  }
