...
$limit(3);
      ...
      { $match: { status: "shipped", placedAt: { $gte: new Date("2026-01-01T00:00:00.000Z") } } },
      { $lookup: { from: "users", localField: "userId", foreignField: "_id", as: "buyer" } },
      { $unwind: "$buyer" },
      { $group: { _id: "$buyer.department", revenue: { $sum: "$total" }, orders: { $sum: 1 } } },
  ...
  it("compiles to the expected MQL", { kind: "pipeline", usage: "db.clients.aggregate(jsmql(...))" }, () => {
    ...
    expect(
      ...
      { $match: { $expr: false } },
      {
        $unionWith: {
          coll: "transactions",
          pipeline: [{ $match: { createdAt: { $gte: new Date("2026-01-01T00:00:00.000Z") }, client: 156 } }],
        },
      },
    ]);
  });
...
describe("narrow the current stream (`$$.filter(...)`)", { features: ["Pipelines"] }, () => {
  it("compiles to the expected MQL", { kind: "pipeline", usage: "db.transactions.aggregate(jsmql(...))" }, () => {
    ...
    expect(jsmql`$$.filter(t => t.createdAt >= new Date("2026-01-01") && t.client === 156);`).toEqual([
      { $match: { createdAt: { $gte: new Date("2026-01-01T00:00:00.000Z") }, client: 156 } },
    ]);
  });
});
...
$project({ total: basePrice });
        `),
      ).toThrow(/'basePrice' is a 'const' and cannot be assigned again/);
    },
  );
});
...
describe("active premium subscribers", { features: ["Filters"] }, () => {
  it("compiles to the expected MQL", { kind: "filter", usage: "db.subscribers.find(jsmql(...))" }, () => {
    expect(
      ...
      status: "active",
      expiresAt: { $gt: new Date("2026-05-01T00:00:00.000Z") },
    });
  });
});
...
$.status === "shipped"
      ...
      customerId: "cust_42",
      placedAt: { $gte: new Date("2026-01-01T00:00:00.000Z"), $lt: new Date("2026-02-01T00:00:00.000Z") },
      status: "shipped",
    });
  });
});
...
describe("archivable docs (not pinned, untouched since)", { features: ["Filters"] }, () => {
  it("compiles to the expected MQL", { kind: "filter", usage: "db.documents.find(jsmql(...))" }, () => {
    expect(jsmql(`$.pinned !== true && $.lastModifiedAt < new Date("2025-01-01")`)).toEqual({
      pinned: { $ne: true },
      lastModifiedAt: { $lt: new Date("2025-01-01T00:00:00.000Z") },
    });
  });
});
...
!($.driver.occupation in ["stunt-double", "test-pilot", "demolition-engineer"])
      ...
      "driver.age": { $gte: 25, $lte: 70 },
      "policy.kmPerYear": { $lte: 30000 },
      $nor: [{ "driver.occupation": { $in: ["stunt-double", "test-pilot", "demolition-engineer"] } }],
    });
  });
});
...
// *user*). The scope-depth-stamped let names keep them distinct: `$$jsmql_f1__id`
// is the order's `_id`, `$$jsmql_f0__id` is the user's. (With a single shared
// un-stamped name they would collide and the user correlation would silently
// read the order's id.)
...
$$$$.dw.archive_users = $$;
      `,
        ...
        { $match: { active: false, lastSeen: { $lt: new Date("2025-01-01T00:00:00.000Z") } } },
...
describe("most expensive line items across shipped orders (`.flatMap` + `.map`)", { features: ["Pipelines"] }, () => {
  it(
    ...
    { kind: "pipeline", usage: "db.orders.aggregate(jsmql(...))" },
    () => {
      // `.flatMap("items")` lowers to `$unwind: "$items"`, and from there on each
      // callback receives the ITEM: `.map(item => item)` replaces each doc with
      // just the item (so the next `$sort` indexes into the item directly).
      // `.flatMap("items")` is the lodash property shorthand (equivalent to
      // `o => o.items`); `.filter({ status: … })` is the matches-object
      // shorthand for `o => o.status === …`.
      expect(
        jsmql`
$$.filter({ status: "shipped" }).flatMap("items").map(item => item);
$sort({ price: -1 });
$limit(50);
        `,
      ).toEqual([
...
$$ = [{
  ...
  smallest:  $$.reduce((acc, o) => Math.min(acc, o.total), 0)
        ...
        {
          $match: {
            placedAt: { $gte: new Date("2026-05-01T00:00:00.000Z"), $lt: new Date("2026-06-01T00:00:00.000Z") },
          },
        },
        {
          $group: {
...
$sort({ revenue: -1 });
        `),
      ).toThrow(/Nothing can follow '\$merge'/);
    },
  );
});
  ...
  it(
    ...
    () => {
      ...
      expect(() => jsmql(`$match({ location: { $near: [-73.9, 40.7] } });`)).toThrow(
        /'\$near' is not allowed inside an aggregation '\$match'.*\$geoNear/s,
      );
    },
  );
});
