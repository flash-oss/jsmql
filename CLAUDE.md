...
src/
  ...
  ops.ts          GENERATED. Ambient `declare global` types for every stage + operator, shipped at the `jsmql/ops` subpath. Regenerated by scripts/generate-ops.mjs on prebuild / pretest.
docs/
...
scripts/
  ...
  sync-playground.mjs           Regenerate playground-examples.json from test/realistic.test.ts
...
- **`playground.html` mirrors `test/realistic.test.ts` via a sidecar JSON manifest.** `scripts/sync-playground.mjs` extracts the first `jsmql(...)` query from each top-level `describe` in `test/realistic.test.ts` and writes the result to `playground-examples.json`. The playground page fetches that file at load time — the queries themselves live only in the test file, not duplicated inline in the HTML. A PostToolUse hook in `.claude/settings.json` runs the script (and `git add`s the manifest) whenever Claude Code edits `test/realistic.test.ts`. Outside Claude Code, run `npm run sync:playground` after editing the test file.
