...
The public API is the `jsmql` callable from `src/index.ts`, which carries two properties (`jsmql.compile`, `jsmql.validate`). The shape is built with `Object.assign` rather than a `namespace` declaration because `src/` stays in TypeScript's strippable subset — see [src/index.ts:271-284](src/index.ts:271).
...
jsmql targets aggregation expressions and pipeline stages — including the index-friendly query-document form of `$match`. It does **not** compile to `db.collection.find()` filter documents; query-predicate operators inside `find()` are flagged as future work in `docs/CLAUDE.md`.
