...
3. If the operator's result type is the same whatever its arguments are, add an `OPERATOR_RETURNS` row — read the category off a running `mongod` with `{ $type: { <op>: <args> } }`, never from the vendored YAML's `type:` field (it is wrong for `$trunc`). Leave the row out when the type depends on the arguments; absence is what keeps the chain type-check from rejecting valid code.
4. Add at least one test case in `test/codegen.test.ts`.
5. If the operator has user-visible syntax (e.g. a named convenience form), update `docs/LANGUAGE.md`.
6. Update `docs/specs/operator-registry.md` if shape semantics change. The drift-protection test (`test/operator-spec-coverage.test.ts`) will catch missing categories or descriptions.

### Adding a JS-method alias (`.foo()`)
1. Write ONE declaration in the matching family file under `src/methods/` — receiver family, `args` rule, `value` lowering (or `unsupported(reason)`). **Never add a `case` to the `generateMethodCall` switch**: a ratchet in `test/methods-grid.test.ts` fails if the un-migrated count rises, and that is the habit the grid exists to break. What the lowering needs from the compiler arrives through `LowerInput` as a service, never as an import. The `args` rule is the arity check — never hand-roll an `if (length …) throw`. See [docs/specs/lowering-grid.md](docs/specs/lowering-grid.md).
2. Add a `foo: { returns?, optional? }` entry to the `METHODS` registry (`src/codegen.ts`). Set `returns` when the result type is invariant (drives string/array/bool inference and the chain type-check's view of `.foo()` as a *receiver*), `optional` to the receiver type for `?.`-chain neutrals. This single entry also adds `foo` to the `didYouMean` suggestion list — there are no separate Sets to update. **If the method only applies to one receiver type, it MUST declare that family** — `optional` for string/array, `NUMBER_RECEIVER_METHODS` / `OBJECT_RECEIVER_METHODS` for number/object, `receiver: "date"` for date — otherwise the chain type-check can't gate it and `$.s.trim().foo()` emits MQL the server refuses. Declare it even when the lowering only throws (the mutator/void shims do), so the type error precedes the shim's advice. A completeness test asserts that the ungated set holds only the genuinely dual/universal methods.
