...
- **`src/` stays in TypeScript's strippable subset** so the source runs as-is on Node 24+ (native type-stripping, no flag), Deno, and Bun. No `enum` (use `as const` objects + derived unions), no `namespace`, no parameter properties (write explicit fields + assignment in the constructor body), no decorators, no `<T>x` casts (use `x as T`), no `import =` / `export =`. Internal imports use `.ts` extensions; `tsconfig.json` carries `allowImportingTsExtensions` + `rewriteRelativeImportExtensions` so `tsc` still emits `.js` to `dist/` for the published package. Test the invariant with `node src/index.ts` — it must run without errors. The canonical strippable patterns live in [`src/lexer.ts`](src/lexer.ts) (`TokenType` as const object, error class with explicit field declaration).
