...
The public API is the `jsmql` callable from `src/index.ts`, which carries three properties (`jsmql.compile`, `jsmql.validate`, `jsmql.expr`). The shape is built with `Object.assign` rather than a `namespace` declaration because `src/` stays in TypeScript's strippable subset — see [src/index.ts:271-284](src/index.ts:271).
