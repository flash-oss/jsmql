...
src/
  ...
  stream-methods.ts      Registry of chainable JS-array-shaped methods (currently `.slice`, `.concat`, `.map`, `.toSorted`) that may appear after the receiver inside a `$$ = $$.<chain>;` or `$$ = $$$.<coll>.<chain>;` RHS. Each entry declares an arg-shape validator and a `lower(args, ctx, callPos, lowerBlock) → { stages, clearLets? }`. Walked by `lowerChainOnStream` / `lowerChainOnCollection` in `pipeline.ts`. `.concat` routes through `lowerUnionPush` (shared with `$$.push`); `.map` lowers to `$replaceWith`; `.toSorted` parses an `(a, b) => …` comparator into a `$sort` spec. See docs/specs/stream-methods.md.
