...
scripts/
  ...
  sync-playground.mjs           Regenerate playground.html examples from test/realistic.test.ts
...
- **`playground.html` mirrors `test/realistic.test.ts`.** The `<option>` list and `<script type="text/plain" data-ex=…>` example blocks live between `<!-- BEGIN/END GENERATED OPTIONS -->` and `<!-- BEGIN/END GENERATED EXAMPLES -->` markers and are regenerated by `scripts/sync-playground.mjs`. A PostToolUse hook in `.claude/settings.json` runs the script (and `git add`s `playground.html`) whenever Claude Code edits `test/realistic.test.ts`, so the two files stay in step within one commit. Outside Claude Code, run `npm run sync:playground` after editing the test file.
