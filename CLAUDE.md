...
src/
  ...
  stream-methods.ts      Registry of chainable JS-array-shaped methods (currently `.slice(start, end?)`) that may appear after the receiver inside a `$$ = $$.<chain>;` or `$$ = $$$.<coll>.<chain>;` RHS. Each entry declares an arg-shape validator and a `lower(args, ctx, callPos) → { stages, clearLets? }`. Walked by `lowerChainOnStream` / `lowerChainOnCollection` in `pipeline.ts`. See docs/specs/stream-methods.md.
