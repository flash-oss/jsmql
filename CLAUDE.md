...
src/
  ...
  stream-methods.ts      Registry of chainable JS-array-shaped methods (currently `.slice`, `.concat`, `.map`, `.toSorted`, `.toReversed`, `.flatMap`, `.reduce`) that may appear after the receiver inside a `$$ = $$.<chain>;` or `$$ = $$$.<coll>.<chain>;` RHS. Each entry declares an arg-shape validator and a `lower(args, ctx, callPos, lowerBlock, prevStages) → { stages, clearLets?, replacesPreviousStage? }`. Walked by `lowerChainOnStream` / `lowerChainOnCollection` in `pipeline.ts`. `.toReversed` reads `prevStages` to flip the preceding `$sort`; `.flatMap(d => d.<path>)` lowers to `$unwind`; `.reduce((acc, d) => …, init)` on `$$` lowers to `$group { _id: null, value: { $sum/$max/$min: … } }` (distinct from the lookup-chain `.reduce` terminal in lookup-translation.ts). See docs/specs/stream-methods.md.
