...
src/
  ...
  stream-methods.ts      Registry of chainable JS-array-shaped methods (currently `.slice`, `.concat`, `.map`, `.toSorted`, `.toReversed`) that may appear after the receiver inside a `$$ = $$.<chain>;` or `$$ = $$$.<coll>.<chain>;` RHS. Each entry declares an arg-shape validator and a `lower(args, ctx, callPos, lowerBlock, prevStages) → { stages, clearLets?, replacesPreviousStage? }`. Walked by `lowerChainOnStream` / `lowerChainOnCollection` in `pipeline.ts`. `.toReversed` reads `prevStages` and returns `replacesPreviousStage: true` to flip the preceding `$sort`. See docs/specs/stream-methods.md.
