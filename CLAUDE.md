...
src/
  ...
  operators.ts    MongoDB operator shapes + categories, read by the globals generator (the registry rows in src/registry/names.ts are the compiler's source of truth).
  stages.ts       Aggregation stage shapes, read by the globals generator.
...
3. Add an entry to `OPERATORS` in `src/operators.ts` (the globals generator's shape/category source) with a `category` from `OPERATOR_CATEGORIES` and the same description.
4. Add a case to the matching `test/compiler-*.test.ts` suite — run it on mongod where a shape's validity is in doubt.
5. If the operator has user-visible syntax, update `docs/LANGUAGE.md`. The drift tests (`test/operator-spec-coverage.test.ts`, `test/registry-agrees.test.ts`) catch a missing description or category.
