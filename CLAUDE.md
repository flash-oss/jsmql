...
- **Errors stay consistent and helpful across the surface.** When you add a new throw site, match the patterns the existing ones already use — don't invent a one-off phrasing for one error category that's worded differently from its siblings. Concretely:
  ...
  - `.validate()` errors must always carry a meaningful `.pos`. The `ValidationError` shape declares `.pos: number` as part of the public contract — tooling (editor integrations, the playground) uses it to underline the offending region, and `.pos = 0` as a placeholder defeats that contract. When you add a new throw site that can reach `.validate()`, thread real position information through to the error. Lex/parse throws already do this from token offsets; codegen and adapter throws (`CodegenError`, `FunctionInputError`, `JsmqlInterpolationError`) currently fall back to `0` because AST nodes don't preserve position — extending those layers to carry source offsets is the right fix, not the workaround.
