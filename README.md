...
- **A dot runs the method on an empty collection; `?.` gives `null`** — an array or object method on a field that is null or missing runs on `[]` or `{}`: `$.tags.uniq()` is `[]`, `$.tags.has("red")` is `false`, `$.o.pick(["a"])` is `{}`, so a computed column has the type you expect in every row. A string method answers `null` there. Write `?.` and the chain stops with `null` instead, as JavaScript's `?.` does. Each method reads one kind of value, and its name says which: `.length()` counts characters, `.size()` counts elements, `.includes()` tests a substring, `.has()` tests membership. See [docs/LANG_RULES.md](docs/LANG_RULES.md) HR5.
