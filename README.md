...
// → { $expr: { $in: ["$userId", { $concatArrays: [{ $ifNull: ["$mods", []] }, { $ifNull: ["$room.mods", []] }, ["root"]] }] } }
...
- **Diagnostics scoped by prefix** — a system source stage is a method call on the context reference with the scope it needs: `$$.indexStats()` and similar calls need the current collection; `$$$$.currentOp({…})` and similar calls need the deployment. The wrong scope is a compile-time error that names the right prefix. See [docs/LANGUAGE.md → System / diagnostic stages](docs/LANGUAGE.md#system--diagnostic-stages-indexstats-currentop-).
...
- **A dot runs the method on an empty array or object; `?.` gives `null`** — an array or object method on a field that is null or missing runs on `[]` or `{}`: `$.tags.uniq()` is `[]`, `$.tags.has("red")` is `false`, `$.o.pick(["a"])` is `{}`, so a computed column has the type you expect in every row. A string method answers `null` there. Write `?.` and the chain stops with `null` instead, as JavaScript's `?.` does. Each method reads one kind of value, and its name says which: `.length()` counts characters, `.size()` counts elements, `.includes()` tests a substring, `.has()` tests membership. See [docs/LANG_RULES.md](docs/LANG_RULES.md) HR5.
