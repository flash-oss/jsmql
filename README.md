...
- **JS you already know** — operators, ternaries, template literals, optional chaining, spread, computed keys, numeric separators, `Math.*`, `Date`, `typeof`, `instanceof`, comments, the `function` keyword, and block-body arrows with local `const`s (`x => { const y = …; return … }` → nested `$let`). If `node --check` accepts it, jsmql does too.
